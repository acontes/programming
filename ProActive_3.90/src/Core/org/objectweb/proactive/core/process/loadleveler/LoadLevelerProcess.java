...
public class LoadLevelerProcess extends AbstractExternalProcessDecorator {
    ...
    private String buildLoadLevelerParameters() {
        ...
        builder.append(buildLoadLevelerComment(" LoadLeveler job description file generated by ProActive"));

        builder.append(buildLoadLevelerComment(" ---- Job Configuration ---- "));

        // usr/bin/poe by default
        builder.append(buildLoadLevelerProperty(LLTAG_EXECUTABLE, this.getExecutable()));

        if (this.getArguments() != null) {
            builder.append(buildLoadLevelerProperty(LLTAG_EXEC_ARGS, this.getArguments()));
        }

        // the java.io.tmpdir by default
        if (this.getInitialDir() != null) {
            builder.append(buildLoadLevelerProperty(LLTAG_INITIAL_DIR, this.getInitialDir()));
        }

        if (this.getOutputFile() != null) {
            builder.append(buildLoadLevelerProperty(LLTAG_STD_OUTPUT_FILENAME, this.getOutputFile()));
        }

        if (this.getErrorFile() != null) {
            builder.append(buildLoadLevelerProperty(LLTAG_ERROR_OUTPUT_FILENAME, this.getErrorFile()));
        }
        ...
        String tmpTaskEnvironment = " $" + PROACTIVE_CMD_TAG + "=" + this.targetProcess.getCommand();

        if (this.getTaskEnvironment() != null) {
            // Prepending user define environement
            tmpTaskEnvironment = this.getTaskEnvironment() + " ; " + tmpTaskEnvironment;
        }
        builder.append(buildLoadLevelerProperty(LLTAG_ERROR_ENVIRONMENT, tmpTaskEnvironment));
        ...
        if (this.getResources() != null) {
            resourcesString = buildLoadLevelerProperty(LLTAG_RESOURCES, this.getResources());
        }

        if (this.isSimpleTaskRepartition()) {
            String consumableCpu = " " + LLTAG_CONSUMABLE_CPUS + "(" + this.cpusPerTasks + ")";
            if (resourcesString == null) {
                resourcesString = buildLoadLevelerProperty(LLTAG_RESOURCES, consumableCpu);
            } else {
        ...
        if (this.isSimpleTaskRepartition()) {
            ...
            if (this.getTotalTasks() != -1) {
                builder.append(buildLoadLevelerProperty(LLTAG_TOTAL_TASKS, "" + this.getTotalTasks()));
            }

            if (this.getTasksPerNode() != -1) {
                builder.append(buildLoadLevelerProperty(LLTAG_TASKS_PER_NODE, "" + this.getTasksPerNode()));
            }

            if (this.getNode() != -1) {
                builder.append(buildLoadLevelerProperty(LLTAG_NODE, "" + this.getNode()));
            }

            if (this.getTaskGeometry() != null) {
                builder.append(buildLoadLevelerProperty(LLTAG_TASK_GEOMETRY, this.getTaskGeometry()));
            }
        }
    ...
    protected boolean isSimpleTaskRepartition() {
        ...
        return ((this.getNbTasks() != -1) && (this.getTasksPerHosts() != -1) && (this.getCpusPerTasks() != -1));
    }

...
public class HostMonitor3D extends AbstractFigure3D implements Figure3D {
	private static final double LOAD_AVERAGE_THRESHOLD = 0.2;
	private static final double LOAD_HIGH_THRESHOLD = 0.8;

	public HostMonitor3D(final String name) {
		super(name);
	...
	public void setLoad(final double loadScale) {
		if ((loadScale > 1) || (loadScale < 0)) {
			throw new IllegalArgumentException("The scale  " + loadScale
					+ " should be in the [0,1] range");
		}

		if (loadScale > HostMonitor3D.LOAD_HIGH_THRESHOLD) {
			this.setAppearance(AppearanceBasket.monitorFull);
		} else if (loadScale > HostMonitor3D.LOAD_AVERAGE_THRESHOLD) {
			this.setAppearance(AppearanceBasket.monitor);
		} else {
			this.setAppearance(AppearanceBasket.monitorLow);
		}

		final TransformGroup translateScaleTransform = this

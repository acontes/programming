...
public final class GeometryBasket {
	private static final Logger logger = Logger.getLogger(GeometryBasket.class
			.getName());
	/**
	 * Contains an application wide value for the size of the figures. It is
	 * needed for placing and scaling the figures.
	 */
	public static final float FIGURE_SCALE = 1f;
	// default geometries
	private static Geometry nodeGeometry = GeometryBasket.nodeGeometry();
	private static Geometry activeObjectGeometry = GeometryBasket
			.activeObjectGeometry();
	private static Geometry gridGeometry = GeometryBasket.gridGeometry();
	private static Geometry hostGeometry = GeometryBasket.hostGeometry();
	private static Geometry alternateHostGeometry = GeometryBasket
			.alternateHostGeometry();
	...
	private static Geometry nodeGeometry() {
		...
		final Point3f c1 = new Point3f(GeometryBasket.FIGURE_SCALE,
				GeometryBasket.FIGURE_SCALE, GeometryBasket.FIGURE_SCALE);
		final Point3f d1 = new Point3f(GeometryBasket.FIGURE_SCALE, 0f,
				GeometryBasket.FIGURE_SCALE);
		...
		// build the geometry
		final GeometryInfo geoInfo = new GeometryInfo(
	...
	private static Geometry hostGeometry() {
		...
		final Point3f c = new Point3f(GeometryBasket.FIGURE_SCALE, 0f,
				GeometryBasket.FIGURE_SCALE);
		final Point3f d = new Point3f(GeometryBasket.FIGURE_SCALE, 0f, 0f);
	...
	private static Geometry alternateHostGeometry() {
		...
		final Point3f a = new Point3f(0f, 0f, 0f);
		final Point3f m1 = new Point3f(0.0f, 0f,
				GeometryBasket.FIGURE_SCALE / 2);
		final Point3f b = new Point3f(0f, 0f, GeometryBasket.FIGURE_SCALE);

		final Point3f m2 = new Point3f(GeometryBasket.FIGURE_SCALE, 0f,
				GeometryBasket.FIGURE_SCALE / 2);
		final Point3f c = new Point3f(GeometryBasket.FIGURE_SCALE, 0f,
				GeometryBasket.FIGURE_SCALE);
		final Point3f d = new Point3f(GeometryBasket.FIGURE_SCALE, 0f, 0f);
		...
		final Color3f[] myColors = { red, red, red, red, green, green, green,
				green, red, red, red, red, green, green, green, green };
		// ------------------------------------------
		// create the points
	...
	private static Geometry barMonitorGeometry() {
		...
		final Point3f a = new Point3f(0f, 0f, 0f);
		final Point3f b = new Point3f(0f, 0f,
				GeometryBasket.FIGURE_SCALE * 0.25f);
		final Point3f c = new Point3f(GeometryBasket.FIGURE_SCALE * 0.25f, 0f,
				GeometryBasket.FIGURE_SCALE * 0.25f);
		final Point3f d = new Point3f(GeometryBasket.FIGURE_SCALE * 0.25f, 0f,
				0f);
		...
		// ------------------------------------------
		// create the points
		final Point3f[] pts = { a, b, c, d, a, d, h, e, a, e, f, b, g, h, d, c,
				c, b, f, g, h, g, f, e // Top
		};
	...
	public static Geometry getDefaultLineGeometry(final Vector3f begin,
			final Vector3f end) {
	...
	// TODO does not work
	public static Geometry sphereArrowGeometry(final Tuple2d ao1,
			final Tuple2d ao2, int segments, final double radius) {
		/*
		...
		GeometryBasket.logger.trace("ao1.x :" + ao1.x + " ao1.y :" + ao1.y
				+ " ao2.x :" + ao2.x + " ao2.y :" + ao2.y);
		// calculate a segments number of points between
		// the angles of ao1, ao2
		for (int i = 0; i < segments; i++) {

			// get the coordinates in a plane
			x = Math.sin(distance1 * i + ao1.x)
					* Math.cos(distance2 * i + ao1.y) * radius;

			y = Math.sin(distance1 * i + ao1.x)
					* Math.sin(distance2 * i + ao1.y) * radius;
			...
			System.out.println((distance1 * i + ao1.x) + ":"
					+ (distance2 * i + ao1.y));
	...
	// not static because lines have different coordinates
	public static Geometry getDefaultCurveGeometry(final Vector3f begin,
			final Vector3f end, final int segments) {
		final Point3f startCoord = new Point3f(begin);
		final Point3f stopCoord = new Point3f(end);
	...
	private static Geometry activeObjectGeometry() {
		final Sphere s = new Sphere(1, Primitive.GENERATE_NORMALS
				| Primitive.GENERATE_TEXTURE_COORDS, 25);

...
public class AnimationBasket {
	 ...
	 */
	public  void fadeInto(AbstractFigure3D figure,
			int runTime){
		TransparencyAttributes attrib = 
			figure.getAppearance().getTransparencyAttributes();
		Alpha alpha = new Alpha(1, runTime);
		TransparencyInterpolator transp = 
			new TransparencyInterpolator(alpha,attrib,
					1,attrib.getTransparency());
	...
	public  void skyDrop(AbstractFigure3D figure,
			int runTime){
		TransformGroup trans = (TransformGroup)figure.getParent();
		Transform3D t3d = new Transform3D();
		...
		t3d.get(translation);
		Alpha alpha = new Alpha(1, runTime);
		PositionInterpolator scale = new PositionInterpolator(alpha,trans,
				t3d,-100,(float)translation.x);
		scale.setSchedulingBounds(new BoundingSphere());
		//the branch group is needed in order to be able
		//to add to the compiled scene
		BranchGroup bg = new BranchGroup();

...
public class TextStylesBasket {
	...
	public static TransformGroup hostText(final String text) {
		return TextStylesBasket.figureText(text, ColorPalette.BLACK, "Arial",
				25, Font.PLAIN, new Vector3d(0d, -0.1d, 0.1d), new AxisAngle4d(
						1, 0, 0, Math.PI / 4), 1);
	}

	public static TransformGroup runtimeText(final String text) {
		return TextStylesBasket.figureText(text, ColorPalette.BLACK,
				Font.SERIF, 20, Font.BOLD, new Vector3d(1.1d, 0.1d, 0.5d),
				new AxisAngle4d(1, 0, 0, Math.PI / 3), 1.5);
	}

	public static TransformGroup activeObjectText(final String text) {
		return TextStylesBasket.figureText(text, ColorPalette.BLACK,
				Font.MONOSPACED, 30, Font.BOLD, new Vector3d(1.5d, -1d, 0d),
	...
	public static TransformGroup nodeText(final String text) {
		return TextStylesBasket.figureText(text, ColorPalette.BLACK, "Arial",
				35, Font.BOLD, new Vector3d(0.1d, 0.1d, 1.1d), new AxisAngle4d(
						0, 0, 0, 0), 1); // no rotation or scaling
	}

	private static TransformGroup figureText(final String text,
			final Color3f color, final String fontName, final int fontSize,
			final int fontStyle, final Vector3d translation,
			final AxisAngle4d rotation, final double scale) {
		final Text2D figureName = new Text2D(text, color, fontName, fontSize,
				fontStyle);

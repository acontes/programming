...
public class TextStylesBasket {
    ...
    public static TransformGroup hostText(String text) {
        return figureText(text, ColorPalette.BLACK, "Arial", 25, Font.PLAIN, new Vector3d(0d, -0.1d, 0.1d),
                new AxisAngle4d(1, 0, 0, Math.PI / 4), 1);
    }

    public static TransformGroup runtimeText(String text) {
        return figureText(text, ColorPalette.BLACK, Font.SERIF, 20, Font.BOLD,
                new Vector3d(1.1d, 0.1d, 0.5d), new AxisAngle4d(1, 0, 0, Math.PI / 3), 1.5);
    }

    public static TransformGroup activeObjectText(String text) {
        return figureText(text, ColorPalette.BLACK, Font.MONOSPACED, 30, Font.BOLD, new Vector3d(1.5d, -1d,
            0d), new AxisAngle4d(1, 0, 0, Math.PI / 2), 8);

    }

    public static TransformGroup nodeText(String text) {
        return figureText(text, ColorPalette.BLACK, "Arial", 35, Font.BOLD, new Vector3d(0.1d, 0.1d, 1.1d),
                new AxisAngle4d(0, 0, 0, 0), 1); //no rotation or scaling		
    }

    private static TransformGroup figureText(String text, Color3f color, String fontName, int fontSize,
            int fontStyle, Vector3d translation, AxisAngle4d rotation, double scale) {
        Text2D figureName = new Text2D(text, color, fontName, fontSize, fontStyle);

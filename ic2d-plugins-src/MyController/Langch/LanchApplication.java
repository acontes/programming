...
public class LanchApplication {
	...
	public void action() throws Exception {
		...
		Factory f = org.objectweb.proactive.core.component.adl.FactoryFactory
				.getFactory();

		// initial the proactive deployment discripter
		ProActiveDescriptor deploymentDescriptor = ProActive.getProactiveDescriptor("myhelloworld/deployment/mydeployment-descriptor.xml");
		...
		do {
			...
			if (input == null)
				...
				if (input.equals("*")) {
					...
					System.out.println();
					// add some control of testcase here
					((Client) testcase.getFcInterface("Outer-interface"))
							.setLocation(input);
					System.out.println(">>> Setting the location to be "
							+ input + "... <<<");
...
//						System.out.println("====> " + t[i]);
	...
	private class ComponenMonitorFresher implements Runnable
	{
		...
		{
			String Space = "";
			for(int i=0;i<hierachicalLevel;i++)Space+="    ";
			try
			{
				ComponentMonitorController CMc = (ComponentMonitorController)c.getFcInterface("monitor-controller");
				System.out.println(Space+"--Component : "+(String)CMc.execMonitor(AbstractComponentMonitorController.COMPONENT_NAME, new Object[1]).getObject());
				String[] metrics = CMc.listMetrics();
				for(String m : metrics)
				{
					String[] param = new String[10];
					...
					System.out.println(Space+"     |--"+"Metric ==>"+m+" : "+(String)CMc.execMonitor(m, new Object[1]).getObject());
				}
				Component[] subComponents = CMc.getSubComponents();
		...
		public void run()
		{
			while(true)
			{
				try
				{
					...
					System.out.println();
					System.out.println("==============Begin to Monitor the component and its childern============");
					ShowComponentInfo(this.root,0);
					System.out.println("=========================Components Monitoring Ended=====================");
					System.out.println();

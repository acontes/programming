...
public abstract class KeystoreUtils {
    ...
    public static CertificateTreeList listKeystore(KeyStore store)
        ...
        while (aliases.hasMoreElements()) {
            list.add(CertificateTree.newTree(KeyStoreTools.getCertificateChain(
                        store,
                        KeyStoreTools.getCertificate(store,
                            aliases.nextElement()))).getRoot());
        }
        return list;
    }

    public static void saveKeystore(String path, String password,
        CertificateTreeList ctl, Map<CertificateTree, Boolean> keepPrivateKeyMap)
        throws KeyStoreException, NoSuchProviderException,
            NoSuchAlgorithmException, CertificateException, IOException,
            UnrecoverableKeyException {
    ...
    private static void addTreeToKeystore(KeyStore store, CertificateTree tree,
        ...
        if (keepPrivateKeyMap.containsKey(tree)) {
            ...
            case APPLICATION:
                if (oneApp) {
                    KeyStoreTools.newApplicationPrivateKey(store,
                        tree.getCertificate());
                } else {

...
public class PolicyEditor extends ViewPart {
	 ...
	 * PolicyEditor.this.fillContextMenu(manager); } }); Menu menu =
	 * menuMgr.createContextMenu(this.viewer.getControl());
	 ...
	 * manager.add(this.action1); manager.add(new Separator());
	 * manager.add(this.action2); }
	 * 
	 * private void fillContextMenu(IMenuManager manager) {
	 * manager.add(this.action1); manager.add(this.action2); // Other plug-ins
	 * can contribute there actions here manager.add(new
	 * Separator(IWorkbenchActionConstants.MB_ADDITIONS)); }
	 * 
	 * private void fillLocalToolBar(IToolBarManager manager) {
	 * manager.add(this.action1); manager.add(this.action2); }
	 * 
	 * private void makeActions() { this.action1 = new Action() { @Override
	 * public void run() { showMessage("Action 1 executed"); } };
	 * this.action1.setText("Action 1"); this.action1.setToolTipText("Action 1
	 * tooltip");
	 * this.action1.setImageDescriptor(PlatformUI.getWorkbench().getSharedImages().
	 * getImageDescriptor(ISharedImages.IMG_OBJS_INFO_TSK));
	 * 
	 * this.action2 = new Action() { @Override public void run() {
	 * showMessage("Action 2 executed"); } }; this.action2.setText("Action 2");
	 * this.action2.setToolTipText("Action 2 tooltip");
	 * this.action2.setImageDescriptor(PlatformUI.getWorkbench().getSharedImages().
	 * getImageDescriptor(ISharedImages.IMG_OBJS_INFO_TSK));
	 * this.doubleClickAction = new Action() { @Override public void run() {
	 * ISelection selection = PolicyEditor.this.viewer.getSelection(); Object
	 * obj = ((IStructuredSelection)selection).getFirstElement();
	 * showMessage("Double-click detected on "+obj.toString()); } }; }
	 * 
	 * private void hookDoubleClickAction() {
	 * this.viewer.addDoubleClickListener(new IDoubleClickListener() { public
	 * void doubleClick(DoubleClickEvent event) {
	 * PolicyEditor.this.doubleClickAction.run(); } }); } private void
	 * showMessage(String message) { MessageDialog.openInformation(
	 * this.viewer.getControl().getShell(), "Policy Editor", message); }
	 */
	/**

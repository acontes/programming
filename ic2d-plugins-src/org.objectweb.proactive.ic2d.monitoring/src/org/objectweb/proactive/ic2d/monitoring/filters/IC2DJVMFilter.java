...
public class IC2DJVMFilter extends Filter {
	...
	/**
	 * Returns true if the object is the IC2D's JVM.
	 */
	public boolean filter(AbstractDataObject object) {
		if(! (object instanceof VMObject))
			return false;
		VMObject vm = (VMObject)object;
		try {
			if(vm.getRuntime().getURL().compareTo(RuntimeFactory.getDefaultRuntime().getURL()) == 0)
				return true;

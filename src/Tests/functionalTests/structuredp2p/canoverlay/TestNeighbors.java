...
public class TestNeighbors {
	...
	public static void initTest() {
		try {
			TestNeighbors.entryPoint = (Peer) PAActiveObject.newActive(
					Peer.class.getName(), new Object[] { OverlayType.CAN });
			TestNeighbors.neighbor = (Peer) PAActiveObject.newActive(Peer.class
	...
	public void testJoin() {
		...
		CANOverlay entryPointOverlay = (CANOverlay) TestNeighbors.entryPoint
				.getStructuredOverlay();
		CANOverlay neighborOverlay = (CANOverlay) TestNeighbors.neighbor
				.getStructuredOverlay();

		Assert.assertTrue(entryPointOverlay.getNeighbors().hasNeighbor(
				TestNeighbors.neighbor));
		Assert.assertTrue(neighborOverlay.getNeighbors().hasNeighbor(
				TestNeighbors.entryPoint));

		// tests with split areas !
		try {
			Assert.assertTrue(new Area().equals(entryPointOverlay.getArea()
	...
	public void testLeave() {
		...
		Assert.assertEquals(nbNeighbors - 1, entryPointOverlay.getNeighbors()
				.size());

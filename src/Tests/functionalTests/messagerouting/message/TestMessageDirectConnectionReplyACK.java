...
public class TestMessageDirectConnectionReplyACK extends MessageFunctionalTest{
	...
	public void test() {
		...
		try{
			...
			Assert.fail("There is a problem in the " + MessageType.DIRECT_CONNECTION_ACK + " implementation:" +
		...
		protected void testInvalidMessage() {
			...
			try {
	            ...
	            Assert.fail("Problem with " + type + " implementation: Attemp to reconstruct the message " + m +
	                " with a corrupted " + DirectConnectionReplyACKMessage.Field.PORT + " field " +
			...
			try {
					...
					try {
						...
						Assert.fail("Problem with " + type + " implementation: Attemp to reconstruct the message " + m +
				                " with a corrupted " + DirectConnectionReplyACKMessage.Field.IP_ADDR + " field " +
				                " from its raw byte array representation actually succeeded!");
					} catch (MalformedMessageException e) {
						// success
					}

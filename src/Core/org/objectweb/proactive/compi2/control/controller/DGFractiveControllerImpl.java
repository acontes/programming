...
public class DGFractiveControllerImpl extends AbstractProActiveController implements DGFractiveController{
	...
	public boolean createInnerComponents(MPISpmd mpiSpmd, Map<String, Object> context){
		...
		try {
			...
			nodes = new Component[allNodes.length];
			...
			try {
				...
				//get ID from the attributeController or somewhere else...
	...
	public boolean orderInnerComponentsAndBindThem(){
		...
		Component[] orderedNodes = new Component[nodes.length];

		try {
			
			for (int i = 0; i< nodes.length; i++){
				orderedNodes[((DGNode)nodes[i]).getRank()] = nodes[i];
			}
			...
			for (int i = 0; i< nodes.length; i++){
				ownerContentController.addFcSubComponent(orderedNodes[i]);
				Fractal.getBindingController(owner).bindFc("outMxNServerItf", orderedNodes[i].getFcInterface("serverItf"));
				Fractal.getBindingController(orderedNodes[i]).bindFc("clientItf", owner.getFcInterface("inMxNClientItf"));
			}

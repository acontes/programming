...
int get_data_payload_size (msg_t * msg){
	int other_info = sizeof(msg_t) - (sizeof(msg->TAG) + sizeof(msg->data_backend));
...
int get_data_payload_size_splitted_msg(splitted_msg_t * msg){
	...
	return (sizeof(*msg) - (sizeof(msg->TAG) + sizeof(msg->msg_type) + sizeof(msg->data_length))); 
}
...
int is_splittable_msg(msg_t * recv_msg_buf) {
	return (get_data_payload_size(recv_msg_buf) <
				get_proactive_buffer_length(recv_msg_buf->count, recv_msg_buf->pa_datatype));
}
...
int recv_ipc_message(int qid, long int tag, msg_t * recv_msg) { 
	int err;
	if ((err = recv_raw_msg_from_ipc_queue(qid, tag, recv_msg, get_payload_size(recv_msg))) < 0) {
			return err;
	}
	...
	// the message is splitted in several part we need to get remaining parts from the queue.
			int data_size_to_recv = get_proactive_buffer_length(recv_msg->count, recv_msg->pa_datatype);
			int receive = 1;
			char * data_ptr_save;
			char * data_ptr;
			...
			while (receive == 1) {
				if ((err = recv_raw_msg_from_ipc_queue(qid, tag, &recv_splitted_msg_buf, get_payload_size_splitted_msg(&recv_splitted_msg_buf))) < 0) {
					return err;
				}
				
				// memcpy retrieved data message
				memcpy(data_ptr, recv_splitted_msg_buf.data, recv_splitted_msg_buf.data_length);
...
int populate_msg(msg_t * send_msg_buf, int msg_type, long int TAG, void * buf, int count, MPI_Datatype datatype, 
					  int src, int dest, int tag, int idjob) {
	int length;	
	// assert lenbgth < MSG_DATA_SIZE
...
int send_splitted_message(int qid, msg_t * send_msg_buf) {
	...
	datatype_size_unit = get_proactive_buffer_length(1, send_msg_buf->pa_datatype);
...
//		nb_elem_to_send  = (remaining_elt_to_send % max_elt_in_msg);
		...
		if(first_run == 1) {
			...
			if ((err = populate_splitted_message(&splitted_msg, msg_type, send_msg_buf->TAG /*TAG*/, buffer_ind, 
			...
			if ((err = send_raw_msg_to_ipc_queue(qid, &splitted_msg, get_payload_size_splitted_msg(&splitted_msg))) < 0) {
				return err;
			}
...
int send_to_ipc(int qid, int msg_type, long int TAG, void * buf, int count, MPI_Datatype datatype, int src, int dest, int tag, int idjob) {
	...
	int ret = populate_msg(&send_msg_buf, msg_type, TAG, buf, count, datatype, src, dest, tag, idjob);
	if (ret < 0) {
		return ret;
	}

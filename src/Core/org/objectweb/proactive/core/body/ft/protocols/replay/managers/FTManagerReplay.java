 ...
 * @since ProActive 2.2
 */
public class FTManagerReplay extends org.objectweb.proactive.core.body.ft.protocols.FTManager {
    ...
    //logger
    protected static Logger logger = ProActiveLogger.getLogger(Loggers.FAULT_TOLERANCE_REPLAY);
    ...
    // pool of MessageInfo
    private MessageInfoReplay forSentRequest;
    private MessageInfoReplay forSentReply;
    ...
    public int init(AbstractBody owner) throws ProActiveException {
        ...
        logger.info(" Replay fault-tolerance is enabled for body " + this.ownerID);
        return 0;
    }
    ...
    public synchronized int onDeliverReply(Reply reply) {
        ...
        if (this.isSignificant(reply)) {
            MessageInfoReplay mi = (MessageInfoReplay) reply.getMessageInfo();
    ...
    public synchronized int onDeliverRequest(Request request) {
        ...
        if (this.isSignificant(request)) {
            ...
            MessageInfoReplay mi = (MessageInfoReplay) request.getMessageInfo();

            // history closure
            this.updateHistory(mi.historyIndex);
            //is there any corresponding awaited request ?
            if (!(this.updateAwaitedRequests(request))) {
                if (FTManagerReplay.isOCEnable || this.completingCheckpoint) {
    ...
    public synchronized int onSendReplyBefore(Reply reply) {
        ...
        // output commit
        if (FTManagerReplay.isOCEnable && this.isOutputCommit(reply)) {
    ...
    public synchronized int onSendRequestBefore(Request request) {
        ...
        // output commit
        if (FTManagerReplay.isOCEnable && this.isOutputCommit(request)) {
            if (logger.isDebugEnabled()) {
    ...
    public synchronized int onSendReplyAfter(Reply reply, int rdvValue, UniversalBody destination) {
        ...
        if (rdvValue == FTManagerReplay.RESEND_MESSAGE) {
            try {
    ...
    public int onServeRequestBefore(Request request) {
        ...
        if (FTManagerReplay.isOCEnable) {
            ...
            if (mi != null) {
                long requestIndex = ((MessageInfoReplay) (mi)).positionInHistory;
    ...
    public int beforeRestartAfterRecovery(CheckpointInfo ci, int inc) {
        CheckpointInfoReplay cic = (CheckpointInfoReplay) ci;
        ...
        //System.out.println("[Replay] enable communication");
     ...
     */
    private void sendLogs(CheckpointInfoReplay ci) {
        //send replies
        //System.out.println("[Replay] Sending logged messages...");
    ...
    private void filterQueue(BlockingRequestQueue queue, CheckpointInfoReplay cic) {
        ...
        while (itQueue.hasNext()) {
            ...
            MessageInfoReplay mi = (MessageInfoReplay) current.getMessageInfo();
            if (mi == null) {
    ...
    //    return (FTManagerReplay.runtime.totalMemory() -
    //    FTManagerReplay.runtime.freeMemory()) / 1024;
    //}
    //////////////////////////////////////////////////////////////////////////////////
    ///////// HANDLING EVENTS ////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////////

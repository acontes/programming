 ...
 * @since 2.2
 */
public class CheckpointServerReplay extends CheckpointServerImpl {

    //logger
    protected static Logger logger = ProActiveLogger.getLogger("CheckpointServerForReplay");
    ...
    public synchronized int storeCheckpoint(Checkpoint c, int incarnation) {
        ...
        // updating monitoring
        int index = ((CheckpointInfoReplay) (c.getCheckpointInfo())).checkpointIndex;
    ...
    public List<Checkpoint> getCheckpoints(UniqueID id) {
        return checkpointStorage.get(id);
    }

    public Hashtable<UniqueID, List<Checkpoint>> getCheckpoints() {
    ...
    /**
     * Not implemented for the Replay protocol
    ...
    private boolean checkRecoveryLine() {
        ...
        if ((nextPossible != null) && (nextPossible.getValue() == systemSize)) {
            ...
            while (itKey.hasMoreElements()) {
                ...
                long nextBase = ((CheckpointInfoReplay) (this.getCheckpoint(key, this.recoveryLine + 1)
    ...
    public synchronized void outputCommit(MessageInfo mi) {
        Hashtable<UniqueID, MutableLong> vectorClock = ((MessageInfoReplay) mi).vectorClock;

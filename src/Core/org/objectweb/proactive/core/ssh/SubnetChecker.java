...
public class SubnetChecker {
    ...
    public String getGatewayInformation(String ipAddress, String req) {
        if (req.equalsIgnoreCase("gateway"))
            return getGatewayName(ipAddress);
        if (req.equalsIgnoreCase("port"))
            return getGatewayPort(ipAddress);
        return null;
    }
     ...
     */
    public String getGatewayPort(String ipAddress) {
        SubnetDefinition sd = checkIP(ipAddress);
        if (sd != null)
            return sd.getPort();
        return null;

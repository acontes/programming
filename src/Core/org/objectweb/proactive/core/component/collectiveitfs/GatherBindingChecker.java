                ...
                if (!(serverSideParametersTypes[i] instanceof ParameterizedType)) {
                    ...
                    if (StubObject.class.isAssignableFrom(
                        ...
                        for (int j = 0; j < implementedInterfaces.length;
                                j++) {
                            try {
                                Method serverSideMethodFromImplementedItf = implementedInterfaces[j].getMethod(serverSideMethod.getName(),
                                        serverSideMethod.getParameterTypes());
                                Type[] serverSideMethodFromImplementedItfParametersTypes =
                                    serverSideMethodFromImplementedItf.getGenericParameterTypes();

...
public class PAProxyBuilder {
    ...
    public static byte[] generatePAProxy (String superClazzName) throws NotFoundException, CannotCompileException, IOException {	
      ...
      // empty constructor 
        ...
        while (is.hasNext()) {
        	...
        	Method m = temp.get(key);
        	  ...
        	  if (!Modifier.isPrivate(ctMethod.getModifiers()) &&
        	      ...
        	      System.out.println("adding " + m.getCtMethod().getLongName() + " attr " + Modifier.toString(m.getCtMethod().getModifiers()));
        		  filtered.put(key,m);
        	  } else {
        	      System.out.println("discarding " + m.getCtMethod().getLongName() + " attr " + Modifier.toString(m.getCtMethod().getModifiers()));
        	  }
        }
        ...
        while (is.hasNext()) {
        	Method m = temp.get(is.next());
        	  ...
        	  try {
                  ...
                  System.out.println("PAProxyBuilder.generatePAProxy()" + methodToGenerate.getLongName() + " attr " + Modifier.toString(methodToGenerate.getModifiers()));
        ...
        CtMethod initActivity = CtNewMethod.make(" public void initActivity(org.objectweb.proactive.Body body) { " +
                "java.lang.reflect.Method[] m = " + generatedCtClass.getName().replace('/', '.') + ".class.getDeclaredMethods();" +
        ...
        generatedCtClass.addMethod(CtNewMethod.make("public " + superCtClass.getName().replace('/', '.') + " getTarget() { return this.proxiedModel; }", generatedCtClass));
...
//         
            ...
            if (!Modifier.isNative(ctMethod2.getModifiers()) && 
                    !Modifier.isFinal(ctMethod2.getModifiers()) 
            ) { 

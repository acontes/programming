...
public abstract class KeyStoreTools {
    ...
    public static PrivateKey getSelfPrivateKey(KeyStore keystore) throws UnrecoverableKeyException,
            KeyStoreException, NoSuchAlgorithmException {
        return getPrivateKey(keystore, KEYSTORE_ENTITY_KEY_PATH);
    }

    public static TypedCertificate getApplicationCertificate(KeyStore keystore) throws KeyStoreException,
            UnrecoverableKeyException, NoSuchAlgorithmException {
        return getCertificate(keystore, KEYSTORE_APPLICATION_KEY_PATH);
    }

    public static TypedCertificateList getApplicationCertificateChain(KeyStore keystore)
            throws KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException {
        return getCertificateChain(keystore, getApplicationCertificate(keystore));
    }

    public static PrivateKey getApplicationPrivateKey(KeyStore keystore) throws UnrecoverableKeyException,
            KeyStoreException, NoSuchAlgorithmException {
    ...
    public static TypedCertificate getCertificate(KeyStore keystore, String alias) throws KeyStoreException,
            UnrecoverableKeyException, NoSuchAlgorithmException {
        PrivateKey pk = null;
        if (keystore.isKeyEntry(alias)) {
            pk = getPrivateKey(keystore, alias);
        }
        return new TypedCertificate((X509Certificate) keystore.getCertificate(alias), pathToType(alias), pk);
    }

    public static TypedCertificate getCertificate(KeyStore keystore, EntityType type, String name)
            throws KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException {
        if ((getApplicationCertificate(keystore).getCert() != null) &&
            getApplicationCertificate(keystore).getCert().getSubjectX500Principal().getName().equals(name)) {
    ...
    private static PrivateKey getPrivateKey(KeyStore keystore, String alias)
        ...
        return (PrivateKey) keystore.getKey(alias, PRIVATE_KEY_PASSWORD.toCharArray());
    }

    private static TypedCertificate getParentCertificate(KeyStore keystore, TypedCertificate certificate)
            throws KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException {
        return getCertificate(keystore, certificate.getType().getParentType(), certificate.getCert()
                .getIssuerX500Principal().getName());
    }

    public static TypedCertificateList getCertificateChain(KeyStore keystore, TypedCertificate certificate)
            throws KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException {
        String issuer = certificate.getCert().getIssuerX500Principal().getName();
        String subject = certificate.getCert().getSubjectX500Principal().getName();
        ...
        TypedCertificateList list = getCertificateChain(keystore, getParentCertificate(keystore, certificate));
        list.add(0, certificate);
        return list;
    }

    public static int getLevel(KeyStore keystore, TypedCertificate certificate) throws KeyStoreException,
            UnrecoverableKeyException, NoSuchAlgorithmException {
        String issuer = certificate.getCert().getIssuerX500Principal().getName();
        String subject = certificate.getCert().getSubjectX500Principal().getName();
        if (subject.equals(issuer)) {
    ...
    public static EntityType pathToType(String path) {
        if (path.contains(KEYSTORE_ENTITY_PATH) || path.equals(KEYSTORE_ENTITY_KEY_PATH)) {
            return EntityType.ENTITY;
        }
        if (path.contains(KEYSTORE_APPLICATION_PATH) || path.equals(KEYSTORE_APPLICATION_KEY_PATH)) {

...
/**
 * Used to send the latencies to the leader
 */
public class MethodCallLatency extends MethodCallControlForGroup {

	private int senderRank;
	private Latency[] latencies;
	 ...
	 */
	public MethodCallLatency(int sender, Latency[] latencies) {
		this.senderRank = sender;
	...
	public Object execute(Object target) throws InvocationTargetException, MethodCallExecutionFailedException {
		try {
			...
			if (AbstractBody.class.isAssignableFrom(uBody.getClass())) {
				...
				if(spmdManager != null){
					ProActiveSPMDTopologyManager topoManager = spmdManager.getTopologyManager();
					if(topoManager != null)
						topoManager.setCommunicationLatencies(senderRank, latencies);
				}
			}
		} catch (Throwable e) {
			e.printStackTrace();

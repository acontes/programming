 ...
 */
public class ProActiveVisitor extends TreePathScanner<Void,Trees> {
	
	private Messager _compilerOutput;
	
	public ProActiveVisitor(Messager messager) {
		_compilerOutput = messager;
	}
	
	@Override
	public Void visitClass(ClassTree clazzTree, Trees trees) {
		//System.out.println("Visiting the class " + clazzTree.getSimpleName() );getCurrentPath();
	...
	public Void visitReturn(ReturnTree returnNode, Trees trees) {
		...
		if (returnNode.getExpression().getKind().equals(Tree.Kind.NULL_LITERAL)) {

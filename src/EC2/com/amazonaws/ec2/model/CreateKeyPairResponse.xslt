        ...
        <xsl:element name="CreateKeyPairResponse" namespace="{$ns}">
            ...
            <xsl:element name="CreateKeyPairResult" namespace="{$ns}">
                <xsl:element name="KeyPair" namespace="{$ns}">
                    <xsl:element name="KeyName" namespace="{$ns}"><xsl:value-of select="ec2:keyName"/></xsl:element>
                    <xsl:element name="KeyFingerprint" namespace="{$ns}"><xsl:value-of select="ec2:keyFingerprint"/></xsl:element>
                    <xsl:element name="KeyMaterial" namespace="{$ns}"><xsl:value-of select="ec2:keyMaterial"/></xsl:element>
                </xsl:element>
            </xsl:element>
        </xsl:element>
    </xsl:template>

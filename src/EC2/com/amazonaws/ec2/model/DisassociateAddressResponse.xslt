...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ec2="http://ec2.amazonaws.com/doc/2008-05-05/" exclude-result-prefixes="ec2">
    ...
    <xsl:template match="ec2:DisassociateAddressResponse">
        <xsl:apply-templates select="ec2:return"/>
    </xsl:template>
    <xsl:template match="ec2:return">
        <xsl:if test=". = 'true'">
            <xsl:element name="DisassociateAddressResponse" namespace="{$ns}">
                <xsl:element name="ResponseMetadata" namespace="{$ns}">
                    <xsl:element name="RequestId" namespace="{$ns}">NA</xsl:element>
                </xsl:element>
            </xsl:element>
        </xsl:if>
        <xsl:if test="not(. = 'true')">
            <xsl:element name="ErrorResponse" namespace="{$ns}">
                <xsl:element name="Error" namespace="{$ns}">
                    ...
                    <xsl:element name="Message" namespace="{$ns}">IP address cannot be disassociated from the instance</xsl:element>

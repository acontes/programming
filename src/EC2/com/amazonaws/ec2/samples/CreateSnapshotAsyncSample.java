 ...
 *
 */
public class CreateSnapshotAsyncSample {
     ...
     */
    public static void invokeCreateSnapshot(AmazonEC2 service, List<CreateSnapshotRequest> requests) {
        ...
        for (CreateSnapshotRequest request : requests) {
            responses.add(service.createSnapshotAsync(request));
        }
        for (Future<CreateSnapshotResponse> future : responses) {
            ...
            try {
                ...
                CreateSnapshotRequest originalRequest = requests.get(responses.indexOf(future));

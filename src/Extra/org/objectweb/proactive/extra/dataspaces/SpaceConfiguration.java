...
/**
 * Stores information needed to configure an instance of a data space, such as
 * remote access URL, local access path and hostname, space type. Subclasses may
 * define some additional information.
 * <p>
 * It is possible to have an instance of this class without remote access URL
 * specified, but this instance is said to be incomplete (see
 * {@link #isComplete()}) - can be used only temporary during configuration
 * process until remote access URL is specified.
 */
public abstract class SpaceConfiguration {
	...
	/**
	 * Creates data space configuration instance. This configuration may be
	 * incomplete (see {@link #isComplete()}), but at least one access method
	 ...
	 * @param path
	 *            Local path to access data. This path is local to host with
	 *            hostname specified in <code>hostname</code> argument. May be
	...
	/**
	 * @return local access path, valid on host with hostname as in
	 *         <code>hostname</code>; may be <code>null</code>
	 */
	...
	/**
	 * @return hostname where data can be accessed locally through local path;
	 *         may be <code>null</code> if {@link #getPath()} is
	 *         <code>null</code>
	 */
	...
	/**
	 * Checks whether this space configuration completely defines data space, so
	 * it can be used to create data space instance - at least space type and
	 * access URL must be specified.
	 * 
	 * @return <code>true</code> if data space is completely defined,
	 *         <code>false</code> if it miss remote access URL specification
	 */

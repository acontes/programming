...
/**
 * Represents any valid URI used in Data Spaces - abstract location in Data
 * Spaces valid across set of nodes.
 * 
 * <p>
 * Example URI:
 * 
 ...
 * <li>name of input/output OR scratch runtime and node id; e.g. stats OR
 * runtimeXX/nodeZZ</li>
 * <li>defined path within data space; e.g. some_dir/file.txt</li>
 * </ol>
 * 
 * Every component except scheme and application id can be unspecified in URI.
 * However, components hierarchy must be obeyed - if higher component is not
 * specified, then all lower components must be unspecified.<br>
 * All described components can be directly accessed through methods.
 * 
 * <p>
 * URI is said to be <strong>complete</strong>, when all its elements are
 * specified, except optional path. Complete URI always points to concrete data
 * space.
 * 
 * <p>
 * Instances of this class are comparable, in a way corresponding to described
 * hierarchy, <code>equals</code> and <code>hashCode</code> methods are also
 * defined.
 * 
 * <p>
...
public final class DataSpacesURI implements Serializable, Comparable<DataSpacesURI> {
	 ...
	 * 
	 * This method is only a shortcut for {@link #createURI(long, SpaceType)}
	 * with <code>null</code> spaceType argument.
	 * 
	 * @param appId
	 *            application id
	 ...
	 *            application id
	 * @param runtimeId
	 *            runtimeId where scratch space is located. May be
	 *            <code>null</code>.
	 * @return URI for that specification
	 */
	...
	/**
	 * Creates URI of scratch type with only runtimeId and nodeId specified.
	 * Created URI is always complete if arguments are not <code>null</code>.
	 * 
	 * This is only a shortcut for
	 * {@link #createScratchSpaceURI(long, String, String, String)} with
	 ...
	 * @param runtimeId
	 *            runtimeId where scratch space is located. May be
	 *            <code>null</code> if following argument nodeId is also
	 *            <code>null</code>.
	 * @param nodeId
	 *            nodeId (for node being on runtime with runtimeId) where
	 *            scratch space is located. May be <code>null</code>.
	 * @return URI for that specification
	 * @throws IllegalArgumentException
	 *             when <code>null</code> values in arguments do not obey URI
	 *             components hierarchy requirements.
	 */
	...
	/**
	 * Creates URI of scratch type. Created URI is always complete if arguments
	 ...
	 * @param runtimeId
	 *            runtimeId where scratch space is located. May be
	 *            <code>null</code> if following arguments are also
	 *            <code>null</code>.
	 * @param nodeId
	 *            nodeId (for node being on runtime with runtimeId) where
	 *            scratch space is located. May be <code>null</code> if
	 *            following argument path is also <code>null</code>.
	 * @param path
	...
	/**
	 * Creates URI of input or output type with only appId and name specified.
	 * Created URI is always complete if arguments are not <code>null</code>.
	 * 
	 * This method is only a shortcut for
	 * {@link #createInOutSpaceURI(long, SpaceType, String, String)} with
	 * <code>null</code> value for argument path.
	 * 
	 * @param appId
	 *            application id
	 * @param spaceType
	 *            space type - only {@link SpaceType#INPUT} or
	 *            {@link SpaceType#OUTPUT} is allowed here. May be
	 *            <code>null</code> if following argument name is also
	 *            <code>null</code>.
	 * @param name
	...
	/**
	 * Creates URI of input or output type. Created URI is always complete if
	 ...
	 *            <code>null</code>.
	 * @param name
	 *            name of input. May be <code>null</code> if following path
	 *            argument is also <code>null</code>.
	 * @param path
	 *            path within data space. May be <code>null</code>.
	 * @return URI for that specification
	 * @throws IllegalArgumentException
	 ...
	 * 
	 * Input string should conform rules mentioned in class description. Any
	...
	/**
	 * @return name for input and output spaces. May be <code>null</code> for
	 *         input/output, is <code>null</code> for scratch or undefined space
	 *         type.
	 */
	...
	/**
	 * @return runtimeId for scratch space. May be <code>null</code> for
	 *         scratch, is <code>null</code> for input/output or undefined space
	 *         type.
	 */
	...
	/**
	 * @return nodeId for scratch space. May be <code>null</code> for scratch,
	 *         is <code>null</code> for input/output or undefined space type.
	 */
	...
	/**
	 * Checks whether URI is completely defined, i.e. points to concrete data
	 * space.
	 * 
	 * Complete URI has space type and name (or runtimeId/NodeId in case of
	 * scratch) defined. Path specification is optional.
	 * 
	 * @return <code>true</code> if this URI is completely defined.
	 ...
	 * 
	 * @param path
	 *            path to set in newly created URI. May be <code>null</code>.
	...
	/**
	 * Generates next URI key (in sense of comparator of this class) in the same
	 * path level. This is method for <strong>internal usage only</strong>.
	 * 

...
public class Attachment {
	public static final Logger logger = ProActiveLogger
			.getLogger(Loggers.FORWARDING_ROUTER);
	...
	public String toString() {
		...
		sb.append("client="
				+ (client == null ? "unknown" : client.getAgentId()) + " ");
		return sb.toString();
	}
	...
	public void setClient(Client client) {
		if (this.client == null) {
			...
			logger
					.warn("Attachement.setClientId() cannot be called twice. Current client: "
							+ this.client + ", discarded: " + client);
		}
	}
	...
	public void send(ByteBuffer byteBuffer) throws IOException {
		...
		if (logger.isDebugEnabled()) {
			String dstClient = this.client == null ? "unknown" : client
					.getAgentId().toString();
			logger.debug("Sent a " + bytes + " bytes message to client "
					+ dstClient + " with " + this.socketChannel.socket());
		}
	}

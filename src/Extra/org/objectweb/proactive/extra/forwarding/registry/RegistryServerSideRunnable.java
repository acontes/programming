...
//	static final Logger logger = ProActiveLogger.getLogger(Loggers.FORWARDING);
	...
	public void run(){
		...
		try {
			...
			//log and quit
			if (logger.isDebugEnabled()) logger.debug("ST failed while opening server socket listening on port: " + registry.getListeningPort());
			System.exit(-1);
		}

		if (logger.isDebugEnabled()) logger.debug("ST created server socket and listening on port " + registry.getListeningPort());
		while (listening) {
			try {
				new Thread(new RegistrationHandler(serverSocket.accept(), registry)).start();
				if (logger.isDebugEnabled()) logger.debug("ST created new RegistrationHandler");
			} catch (IOException e) {
				//log the fact that a connection wasn't correctly accepted and keep listening
				if (logger.isDebugEnabled()) logger.debug("ST failed while accepting a connection, exception: " + e);
			}
		}
		...
		try {
			...
			if (logger.isDebugEnabled()) logger.debug("ST, a problem occured while closing the serverSocket, exception: " + e);
			e.printStackTrace();
		}
	}

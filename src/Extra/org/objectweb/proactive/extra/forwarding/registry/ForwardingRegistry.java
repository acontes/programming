...
//	static final Logger logger = ProActiveLogger.getLogger(Loggers.FORWARDING);
	...
	public static void main(String[] args) {
		...
		for (int i=0;i<args.length;i++) {
			if (args[i].equals("-port")) {
				...
				if (logger.isDebugEnabled()) logger.debug("FR server port is: " + port);
			}
			else {
		...
		if (logger.isDebugEnabled()) logger.debug("FR.main(), shutdown hook added, going to start() FR");
		registry.start();
	...
	private void start() {
		...
		if (logger.isDebugEnabled()) logger.debug("FR.start(), created and started a new thread for the registry server\n now processing messageQueue");
		...
		while (running) {
			...
			try {
				...
				if (logger.isDebugEnabled() && msg != null) logger.debug("FR.start() dequeued msg: " + msg);
			} catch (InterruptedException e) {
				// The waiting has been interrupted.
				if (logger.isDebugEnabled()) logger.debug("FR.start(), The waiting of the blocking queue was interrupted, exception: " + e);
			}
			if (msg != null) {
				if (logger.isDebugEnabled()) logger.debug("FR.start(), forwarding the following message: \n" + msg.toString());
				forwardMessage(msg);
			}
		}

		//FIXME this line of code can't be reached. Add a shutdown hook or similar solution
		if (logger.isDebugEnabled()) logger.debug("FR.start() going to call FR.stop");
		stop(true);
	}
	...
	private void forwardMessage(ForwardedMessage msg) {
		...
		if (regHandler != null) {
			if (logger.isDebugEnabled()) logger.debug("FR.forwardMessage(), target: " + msg.getTargetID() + " is registered, msg enqueued in OH");
			regHandler.getOutHandler().putMessage(msg);
		}

		// if the target is not registered, create and send an ABORT MSG to the sender
		else {
			if (logger.isDebugEnabled()) logger.debug("FR.forwardMessage(), target: " + msg.getTargetID() + " is not registered, sending back abortMessage to sender: " + msg.getSenderID());
			ForwardedMessage response = ForwardedMessage.abortMessage(msg.getTargetID(), msg.getTargetPort(), msg.getSenderID(), msg.getSenderPort(), "Target unreachable");
			synchronized(registrationHandlerMap) {
			...
			if (regHandler != null)
				...
				if (logger.isDebugEnabled()) logger.debug("FR.forwardMessage(), sender tunnel died in the meantime, not sending abort message");
			}
		}
	}
	...
	protected void stop(boolean softly) { //FIXME: For this function to be called, running must be set to false... But this function is the only one able to set it to false
		...
		// stop the server
		if (logger.isDebugEnabled()) logger.debug("FR.stop(), calling ServTh.stop()");
		server.stop();

		// stop the Registration Handlers
		if (logger.isDebugEnabled()) logger.debug("FR.stop(), calling RH.stopListening() on every mapped RH");
		...
		if (logger.isDebugEnabled()) logger.debug("FR.stop(), forwarding remaining messages");
		msg = messageQueue.poll();
		while (msg != null) {
			if (logger.isDebugEnabled()) logger.debug("FR.stop(), forwarding the following message: \n" + msg);
			forwardMessage(msg);
			msg = messageQueue.poll();
		}

		// stop the OutHandlers, close the RegistrationHandlers' sockets, deallocate the RegistrationHandlers
		if (logger.isDebugEnabled()) logger.debug("FR.stop(), calling RH.stop() on every mapped RH");
		for (RegistrationHandler regHandler : registrationHandlerMap.values()) {
	...
	public void killRegistrationHandler(Object key, boolean softly) {
		...
		if (logger.isDebugEnabled()) logger.debug("FR.killRegistrationHandler(), calling RH.stop() on RH.uniqueId: " + regHandler.getHostId());
		regHandler.stop(softly);
	}

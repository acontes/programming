...
public class RegistrationHandler implements Runnable, ConnectionFailureListener {
	...
	public void run() {
		...
		while (listening) {
			if (!(clientSocketWrapper.getSocket().isClosed())) {
				try {
					...
					if (logger.isDebugEnabled()) logger.debug("RH read msg: " + msg);
				} catch (IOException e) {
				...
				if (msg.getType() == ForwardedMessageType.REGISTRATION) {
					...
					if (logger.isDebugEnabled()) logger.debug("RH handling registration");
					...
					if (registry.isKeyInMap(msg.getSenderID())) {
						...
						stop(false);
						if (logger.isDebugEnabled()) logger.debug("RH killed the pre-existing connection for uniqueID: " + hostId);
					}

					if (logger.isDebugEnabled()) logger.debug("RH, processing to a new registration (new outHandler and new mapping) for uniqueID " + hostId);
					// process to a new registration
					// create new OutHandler
					outHandler = new OutHandler(clientSocketWrapper, this);
					new Thread(outHandler).start();
					if (logger.isDebugEnabled()) logger.debug("RH started the new outHandler for uniqueID " + hostId);
					// add mapping in the HashMap
					registry.putMapping(msg.getSenderID(), this);
					if (logger.isDebugEnabled()) logger.debug("RH added new mapping for uniqueID " + hostId);
				}
				// use this thread as the listening side of this connection
				// the received message is not a REGISTRATION MSG, just forward it
				else {
					if (logger.isDebugEnabled()) logger.debug("RH using current thread as server side of the tunnel to " + hostId);
		...
		if (logger.isDebugEnabled()) logger.debug("RH, calling RegistrationHandler.stop()");
		stop(true);
	}
	...
	protected void stopListening() {
		listening = false;
		if (logger.isDebugEnabled()) logger.debug("RH.stopListening() on uniqueId: " + hostId);
	}
	...
	protected void stop(boolean softly) {
		...
		// stop writing
		if (logger.isDebugEnabled()) logger.debug("RH.stop(), stopping outHandler, closing socket and removing mapping");
		if (outHandler != null)
	...
	public void connectionHasFailed(Exception e) {
		if (logger.isDebugEnabled()) logger.debug("RH.connectionHasFailed tunnel to " + hostId + " has failed, exception: " + e);
		stop(false);
	}

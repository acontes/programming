...
public class ForwardedMessage implements Serializable {
    ...
    public ForwardedMessage(ForwardedMessageType type, Object senderID, int senderPort, Object targetID,
            int targetPort, Object data, int ttl) {
        this.ttl = ttl;
        this.type = type;
    ...
    public String toString() {
        switch (type) {
            ...
            case CONNECTION_REQUEST:
                return ("CONNECTION_REQUEST [from=" + senderID + ":" + senderPort + "; to=" + targetID + ":" +
                    targetPort + "]");
            case CONNECTION_ACCEPTED:
                return ("CONNECTION_ACCEPTED [from=" + senderID + ":" + senderPort + "; to=" + targetID +
                    ":" + targetPort + "]");
            case CONNECTION_ABORTED:
                return ("CONNECTION_ABORTED [from=" + senderID + ":" + senderPort + "; to=" + targetID + ":" +
                    targetPort + "; cause=" + data + "]");
            case DATA:
                //			return ("DATA [from="+senderID+":"+senderPort+"; to="+targetID+":"+targetPort+"; size="+((byte[]) data).length+"]");
                return ("DATA [from=" + senderID + ":" + senderPort + "; to=" + targetID + ":" + targetPort +
                    "; size=" + ((byte[]) data).length + "; data=" + (new String((byte[]) data)) + "]");
    ...
    public static ForwardedMessage registrationMessage(Object senderID) {
        return new ForwardedMessage(ForwardedMessageType.REGISTRATION, senderID, 0, null, 0, null,
            DEFAULT_TTL);
    }

    public static ForwardedMessage connectionMessage(Object senderID, Object targetID, int targetPort) {
        return new ForwardedMessage(ForwardedMessageType.CONNECTION_REQUEST, senderID, getSourcePort(),
            targetID, targetPort, null, DEFAULT_TTL);
    }

    public static ForwardedMessage acceptMessage(Object senderID, int senderPort, Object targetID,
            int targetPort) {
        return new ForwardedMessage(ForwardedMessageType.CONNECTION_ACCEPTED, senderID, senderPort, targetID,
            targetPort, null, DEFAULT_TTL);
    }

    public static ForwardedMessage abortMessage(Object senderID, int senderPort, Object targetID,
            int targetPort, String cause) {
        return new ForwardedMessage(ForwardedMessageType.CONNECTION_ABORTED, senderID, senderPort, targetID,
            targetPort, cause, DEFAULT_TTL);
    }

    public static ForwardedMessage dataMessage(Object senderID, int senderPort, Object targetID,
            int targetPort, byte[] data) {
        return new ForwardedMessage(ForwardedMessageType.DATA, senderID, senderPort, targetID, targetPort,
            data, DEFAULT_TTL);
    }

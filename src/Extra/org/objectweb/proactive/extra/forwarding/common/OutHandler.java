...
public class OutHandler implements Runnable{
	 ...
	 */
	public void run() {
		if(logger.isDebugEnabled()) logger.debug("OutHandler.run(): start loop");
		while(isRunning && !sock.getSocket().isClosed()) {
			...
			try {
				...
				if(logger.isDebugEnabled()) logger.debug("The waiting of the blocking queue was interrupted");
			}
			if(msg != null) {
				if(logger.isDebugEnabled()) logger.debug("message to write: "+msg);
				try {
					writeOnSocket(msg);
					if(logger.isDebugEnabled()) logger.debug("message written.");
				} catch (IOException e) {
	...
	public void putMessage(ForwardedMessage message) {
		if(isRunning && !willClose) {
			try {
				...
				else if(logger.isDebugEnabled()) logger.debug("message queued.");

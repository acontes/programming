...
public class ForwardingAgent implements ConnectionFailureListener {
    ...
    public Socket getSocket(Object id, int port) throws ForwardingException {
        ...
        if (logger.isDebugEnabled())
            logger.debug("Trying to init forwarded connection to " + id + " on port " + port);
        // send connection request
        ClientSocketForwarder fw = localHandler.createNewConnection(ForwardedMessage.connectionMessage(
                uniqueID, id, port));
    ...
    private void initialize() {
        ...
        if (logger.isDebugEnabled())
            logger.debug("Initializating Forwarding Agent");
    }
     ...
     */
    private void connect(Object uniqueID, InetAddress regAddress, int regPort) throws IOException {
        if (logger.isDebugEnabled())
            logger.debug("Connecting to registry on " + regAddress + ":" + regPort);
        ...
        if (logger.isDebugEnabled())
            logger.debug("Connection successful. Registering with ID: " + uniqueID);

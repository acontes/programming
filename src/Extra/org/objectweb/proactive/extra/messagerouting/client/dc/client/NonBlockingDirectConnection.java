 ...
 */
public class NonBlockingDirectConnection implements DirectConnection {
    ...
    public NonBlockingDirectConnection(DirectConnectionManager manager, AgentID remoteAgent)
            throws IOException {
        this.dcMan = manager;
    ...
    public boolean connect(InetSocketAddress remote) throws IOException {
        ...
        if (!this.connectionEstablished) {
            ...
            SelectionKey key = channel.register(this.dcMan.getSelector(), SelectionKey.OP_CONNECT);
            key.attach(this);
        } else {
            // directly register for write() ops
            SelectionKey key = channel.register(this.dcMan.getSelector(), SelectionKey.OP_WRITE);
            key.attach(this);
        }
        // re-enter selection
        this.dcMan.unlockSelector();
    ...
    public void handleConnect(boolean successFlag) {
        ...
        this.dcMan.connectionFinished(remoteAgent, this.connectionEstablished);

...
public class FTBase {// extends Thread {
	...
	public void CompExp(int n, double exponent[]) {
		...
		for (int j = 1; j <= m; j++) {
			...
			for (int i = 0; i <= ln - 1; i++) {
				...
				if (Math.abs(exponent[REAL + idx]) < eps)
					exponent[REAL + idx] = 0;
				if (Math.abs(exponent[IMAG + idx]) < eps)
					exponent[IMAG + idx] = 0;
			}
			ku = ku + ln;
			ln = 2 * ln;
	...
	public void initial_conditions(double u0[], int d1, int d2, int d3) {
		...
		for (int k = 0; k < d3; k++) {
			...
			for (int j = 0; j < d1; j++) {
				...
				for (int i = 0; i < d2; i++) {
					u0[REAL + j * isize3 + i * jsize3 + k * ksize3] = tmp[REAL
							+ i * 2];
					u0[IMAG + j * isize3 + i * jsize3 + k * ksize3] = tmp[IMAG
							+ i * 2];
				}
			}
		}
	...
	protected static int fftblock_default = 4 * 4096, // Size of L1 cache on SGI
														// O2K
			fftblock;

	public void Swarztrauber(int is, int m, int len, int n, double x[],
			int xoffst, int xd1, double exponent[], double scr[]) {
		int i, j = 0, l, mx;
		...
		for (BlockStart = 0; BlockStart < len; BlockStart += fftblock) {
			...
			for (l = 1; l <= m; l += 2) {
				...
				for (i = 0; i <= li - 1; i++) {
					...
					for (k = 0; k <= lk - 1; k++) {
						for (j = BlockStart; j <= BlockEnd; j++) {
							x11[REAL] = x[REAL + j * 2 + (i11 + k) * jsize1
									+ xoffst];
							x11[IMAG] = x[IMAG + j * 2 + (i11 + k) * jsize1
									+ xoffst];
							x21[REAL] = x[REAL + j * 2 + (i12 + k) * jsize1
									+ xoffst];
							x21[IMAG] = x[IMAG + j * 2 + (i12 + k) * jsize1
									+ xoffst];
							scr[REAL + j * isize1 + (i21 + k) * jsize1] = x11[REAL]
									+ x21[REAL];
							scr[IMAG + j * isize1 + (i21 + k) * jsize1] = x11[IMAG]
									+ x21[IMAG];
							scr[REAL + j * 2 + (i22 + k) * jsize1] = u1[REAL]
									* (x11[REAL] - x21[REAL]) - u1[IMAG]
									* (x11[IMAG] - x21[IMAG]);
							scr[IMAG + j * 2 + (i22 + k) * jsize1] = u1[IMAG]
									* (x11[REAL] - x21[REAL]) + u1[REAL]
									* (x11[IMAG] - x21[IMAG]);
						}
					}
				}
				if (l == m) {
					for (k = 0; k < n; k++) {
						for (j = BlockStart; j <= BlockEnd; j++) {
							x[REAL + j * 2 + k * jsize1 + xoffst] = scr[REAL
									+ j * isize1 + k * jsize1];
							x[IMAG + j * 2 + k * jsize1 + xoffst] = scr[IMAG
									+ j * isize1 + k * jsize1];
						}
					}
				} else {
					...
					for (i = 0; i <= li - 1; i++) {
						...
						for (k = 0; k <= lk - 1; k++) {
							for (j = BlockStart; j <= BlockEnd; j++) {
								x11[REAL] = scr[REAL + j * isize1 + (i11 + k)
										* jsize1];
								x11[IMAG] = scr[IMAG + j * isize1 + (i11 + k)
										* jsize1];

								x21[REAL] = scr[REAL + j * isize1 + (i12 + k)
										* jsize1];
								x21[IMAG] = scr[IMAG + j * isize1 + (i12 + k)
										* jsize1];

								x[REAL + j * 2 + (i21 + k) * jsize1 + xoffst] = x11[REAL]
										+ x21[REAL];
								x[IMAG + j * 2 + (i21 + k) * jsize1 + xoffst] = x11[IMAG]
										+ x21[IMAG];

								x[REAL + j * 2 + (i22 + k) * jsize1 + xoffst] = u1[REAL]
										* (x11[REAL] - x21[REAL])
										- u1[IMAG]
										* (x11[IMAG] - x21[IMAG]);
								x[IMAG + j * 2 + (i22 + k) * jsize1 + xoffst] = u1[IMAG]
										* (x11[REAL] - x21[REAL])
										+ u1[REAL]
										* (x11[IMAG] - x21[IMAG]);
							}

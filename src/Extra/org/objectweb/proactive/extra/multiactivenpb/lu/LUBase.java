...
public class LUBase extends Thread {
	...
	protected static final double omega_default = 1.2, tolrsd1_def = .00000001,
			tolrsd2_def = .00000001, tolrsd3_def = .00000001,
			tolrsd4_def = .00000001, tolrsd5_def = .00000001, c1 = 1.4,
			c2 = 0.4, c3 = .1, c4 = 1, c5 = 1.4;
	// ---------------------------------------------------------------------
	// grid
	// ---------------------------------------------------------------------
	...
	// ---------------------------------------------------------------------
	protected int itmax;
	protected double dt, omega, frc, ttotal, tolrsd[] = new double[5],
			rsdnm[] = new double[5], errnm[] = new double[5];
	...
	protected static double ce[] = { 2.0, 1.0, 2.0, 2.0, 5.0, 0.0, 0.0, 2.0,
			...
			1.0, 0.0, 0.0, .1, 3.0, 2.0, 2.0, 2.0, .4, .5, 3.0, 3.0, 3.0, .3,
			.02, .01, .04, .03, .05, .01, .03, .03, .05, .04, .03, .02, .05,
			.04, .03, .5, .4, .3, .2, .1, .4, .3, .5, .1, .3, .3, .5, .4, .3,
			.2 };
	// ---------------------------------------------------------------------
	// timers
	// ---------------------------------------------------------------------
	public static final int t_total = 1, t_rhsx = 2, t_rhsy = 3, t_rhsz = 4,
			t_rhs = 5, t_jacld = 6, t_blts = 7, t_jacu = 8, t_buts = 9,
			t_add = 10, t_l2norm = 11, t_last = 11;
	...
	public void setupThreads(LU lu) {
		...
		for (int ii = 0; ii < num_threads; ii++) {
			rhscomputer[ii] = new RHSCompute(lu, partition1[ii][0],
			...
			lowerjac[ii] = new LowerJac(lu, partition2[ii][0],
					partition2[ii][1]);
			lowerjac[ii].id = ii;
			lowerjac[ii].neighbor = lowerjac;
			//lowerjac[ii].start();

			upperjac[ii] = new UpperJac(lu,
					partition2[num_threads - ii - 1][0], partition2[num_threads
							- ii - 1][1]);
			upperjac[ii].id = ii;

...
public class BTBase extends Thread {
	...
	protected double forcing[], u[], rhs[], cv[], cuf[], q[];
	...
	// constants
	protected static double tx1, tx2, tx3, dt, ty1, ty2, ty3, tz1, tz2, tz3,
			dx1, dx2, dx3, dx4, dx5, dy1, dy2, dy3, dy4, dy5, dz1, dz2, dz3,
			dz4, dz5, dssp, dxmax, dymax, dzmax, xxcon1, xxcon2, xxcon3,
			...
			dz3tz1, dz4tz1, dz5tz1, dnxm1, dnym1, dnzm1, c1c2, c1c5, c3c4,
			c1345, conz1, c1, c2, c3, c4, c5, c4dssp, c5dssp, dtdssp, dttx1,
			dttx2, dtty1, dtty2, dttz1, dttz2, c2dttx1, c2dtty1, c2dttz1,
			comz1, comz4, comz5, comz6, c3c4tx3, c3c4ty3, c3c4tz3, c2iv, con43,
			con16;

	protected static double ce[] = { 2.0, 1.0, 2.0, 2.0, 5.0, 0.0, 0.0, 2.0,
			2.0, 4.0, 0.0, 0.0, 0.0, 0.0, 3.0, 4.0, 0.0, 0.0, 0.0, 2.0, 5.0,
			1.0, 0.0, 0.0, 0.1, 3.0, 2.0, 2.0, 2.0, 0.4, 0.5, 3.0, 3.0, 3.0,
			0.3, 0.02, 0.01, 0.04, 0.03, 0.05, 0.01, 0.03, 0.03, 0.05, 0.04,
			0.03, 0.02, 0.05, 0.04, 0.03, 0.5, 0.4, 0.3, 0.2, 0.1, 0.4, 0.3,
			0.5, 0.1, 0.3, 0.3, 0.5, 0.4, 0.3, 0.2 };

	// timer constants
	public boolean timeron = false;
	public static final int t_rhsx = 2, t_rhsy = 3, t_rhsz = 4, t_xsolve = 6,
			t_ysolve = 7, t_zsolve = 8, t_rdis1 = 9, t_rdis2 = 10, t_add = 11,
	...
	public void setupThreads(BT bt) {
		...
		for (int ii = 0; ii < num_threads; ii++) {
			rhscomputer[ii] = new RHSCompute(bt, partition1[ii][0],
					partition1[ii][1], partition2[ii][0], partition2[ii][1]);
			...
			rhsadder[ii] = new RHSAdder(bt, partition2[ii][0],
					partition2[ii][1]);
			rhsadder[ii].id = ii;
			//rhsadder[ii].start();
		}
	...
	public void exact_solution(double xi, double eta, double zeta,
		...
		for (int m = 0; m < 5; m++) {
			dtemp[m + dtmpoffst] = ce[m + 0 * 5]
					...
					* (ce[m + 1 * 5] + xi
							* (ce[m + 4 * 5] + xi
									* (ce[m + 7 * 5] + xi * ce[m + 10 * 5])))
					+ eta
					* (ce[m + 2 * 5] + eta
							* (ce[m + 5 * 5] + eta
									* (ce[m + 8 * 5] + eta * ce[m + 11 * 5])))
					+ zeta
					* (ce[m + 3 * 5] + zeta
							* (ce[m + 6 * 5] + zeta
									* (ce[m + 9 * 5] + zeta * ce[m + 12 * 5])));
		}
	}
		...
		for (k = 0; k <= grid_points[2] - 1; k++) {
			...
			for (j = 0; j <= grid_points[1] - 1; j++) {
				...
				for (i = 0; i <= grid_points[0] - 1; i++) {
					...
					for (ix = 0; ix <= 1; ix++) {
						exact_solution(ix, eta, zeta, Pface, 0 + 0 * 5 + ix
								* 15);
					...
					for (m = 0; m <= 4; m++) {
						Pxi = xi * Pface[m + 0 * 5 + 1 * 15] + (1.0 - xi)
								* Pface[m + 0 * 5 + 0 * 15];
						Peta = eta * Pface[m + 1 * 5 + 1 * 15] + (1.0 - eta)
								* Pface[m + 1 * 5 + 0 * 15];
						Pzeta = zeta * Pface[m + 2 * 5 + 1 * 15] + (1.0 - zeta)
								* Pface[m + 2 * 5 + 0 * 15];

						u[m + i * isize2 + j * jsize2 + k * ksize2] = Pxi
								+ Peta + Pzeta - Pxi * Peta - Pxi * Pzeta
	...
	public void matvec_sub(double ablock[], int blkoffst, double avect[],
			int avcoffst, double bvect[], int bvcoffst) {

		for (int i = 0; i < 5; i++) {
			bvect[i + bvcoffst] += -ablock[i + 0 * 5 + blkoffst]
					* avect[0 + avcoffst] - ablock[i + 1 * 5 + blkoffst]
					* avect[1 + avcoffst] - ablock[i + 2 * 5 + blkoffst]
					* avect[2 + avcoffst] - ablock[i + 3 * 5 + blkoffst]
					* avect[3 + avcoffst] - ablock[i + 4 * 5 + blkoffst]
					* avect[4 + avcoffst];
		}
	}
	...
	public void matmul_sub(double ablock[], int ablkoffst, double bblock[],
		...
		for (int j = 0; j < 5; j++) {
			...
			cblock[0 + j * 5 + cblkoffst] += -ablock[0 + 0 * 5 + ablkoffst]
					* bblock[0 + j * 5 + bblkoffst]
					- ablock[0 + 1 * 5 + ablkoffst]
					* bblock[1 + j * 5 + bblkoffst]
					- ablock[0 + 2 * 5 + ablkoffst]
					* bblock[2 + j * 5 + bblkoffst]
					- ablock[0 + 3 * 5 + ablkoffst]
					* bblock[3 + j * 5 + bblkoffst]
					- ablock[0 + 4 * 5 + ablkoffst]
					* bblock[4 + j * 5 + bblkoffst];

			cblock[1 + j * 5 + cblkoffst] += -ablock[1 + 0 * 5 + ablkoffst]
					* bblock[0 + j * 5 + bblkoffst]
					- ablock[1 + 1 * 5 + ablkoffst]
					* bblock[1 + j * 5 + bblkoffst]
					- ablock[1 + 2 * 5 + ablkoffst]
					* bblock[2 + j * 5 + bblkoffst]
					- ablock[1 + 3 * 5 + ablkoffst]
					* bblock[3 + j * 5 + bblkoffst]
					- ablock[1 + 4 * 5 + ablkoffst]
					* bblock[4 + j * 5 + bblkoffst];

			cblock[2 + j * 5 + cblkoffst] += -ablock[2 + 0 * 5 + ablkoffst]
					* bblock[0 + j * 5 + bblkoffst]
					- ablock[2 + 1 * 5 + ablkoffst]
					* bblock[1 + j * 5 + bblkoffst]
					- ablock[2 + 2 * 5 + ablkoffst]
					* bblock[2 + j * 5 + bblkoffst]
					- ablock[2 + 3 * 5 + ablkoffst]
					* bblock[3 + j * 5 + bblkoffst]
					- ablock[2 + 4 * 5 + ablkoffst]
					* bblock[4 + j * 5 + bblkoffst];

			cblock[3 + j * 5 + cblkoffst] += -ablock[3 + 0 * 5 + ablkoffst]
					* bblock[0 + j * 5 + bblkoffst]
					- ablock[3 + 1 * 5 + ablkoffst]
					* bblock[1 + j * 5 + bblkoffst]
					- ablock[3 + 2 * 5 + ablkoffst]
					* bblock[2 + j * 5 + bblkoffst]
					- ablock[3 + 3 * 5 + ablkoffst]
					* bblock[3 + j * 5 + bblkoffst]
					- ablock[3 + 4 * 5 + ablkoffst]
					* bblock[4 + j * 5 + bblkoffst];

			cblock[4 + j * 5 + cblkoffst] += -ablock[4 + 0 * 5 + ablkoffst]
					* bblock[0 + j * 5 + bblkoffst]
					- ablock[4 + 1 * 5 + ablkoffst]
					* bblock[1 + j * 5 + bblkoffst]
					- ablock[4 + 2 * 5 + ablkoffst]
					* bblock[2 + j * 5 + bblkoffst]
					- ablock[4 + 3 * 5 + ablkoffst]
					* bblock[3 + j * 5 + bblkoffst]
					- ablock[4 + 4 * 5 + ablkoffst]
					* bblock[4 + j * 5 + bblkoffst];
		}
	}

	public void binvcrhs(double lhss[], int lhsoffst, double c[], int coffst,
			double r[], int roffst) {
		double pivot;
		double coeff;
		...
		lhss[1 + 1 * 5 + lhsoffst] = lhss[1 + 1 * 5 + lhsoffst] - coeff
				* lhss[0 + 1 * 5 + lhsoffst];
		lhss[1 + 2 * 5 + lhsoffst] = lhss[1 + 2 * 5 + lhsoffst] - coeff
				* lhss[0 + 2 * 5 + lhsoffst];
		lhss[1 + 3 * 5 + lhsoffst] = lhss[1 + 3 * 5 + lhsoffst] - coeff
				* lhss[0 + 3 * 5 + lhsoffst];
		lhss[1 + 4 * 5 + lhsoffst] = lhss[1 + 4 * 5 + lhsoffst] - coeff
				* lhss[0 + 4 * 5 + lhsoffst];
		c[1 + 0 * 5 + coffst] = c[1 + 0 * 5 + coffst] - coeff
				* c[0 + 0 * 5 + coffst];
		c[1 + 1 * 5 + coffst] = c[1 + 1 * 5 + coffst] - coeff
				* c[0 + 1 * 5 + coffst];
		c[1 + 2 * 5 + coffst] = c[1 + 2 * 5 + coffst] - coeff
				* c[0 + 2 * 5 + coffst];
		c[1 + 3 * 5 + coffst] = c[1 + 3 * 5 + coffst] - coeff
				* c[0 + 3 * 5 + coffst];
		c[1 + 4 * 5 + coffst] = c[1 + 4 * 5 + coffst] - coeff
				* c[0 + 4 * 5 + coffst];
		r[1 + roffst] = r[1 + roffst] - coeff * r[0 + roffst];

		coeff = lhss[2 + 0 * 5 + lhsoffst];
		lhss[2 + 1 * 5 + lhsoffst] = lhss[2 + 1 * 5 + lhsoffst] - coeff
				* lhss[0 + 1 * 5 + lhsoffst];
		lhss[2 + 2 * 5 + lhsoffst] = lhss[2 + 2 * 5 + lhsoffst] - coeff
				* lhss[0 + 2 * 5 + lhsoffst];
		lhss[2 + 3 * 5 + lhsoffst] = lhss[2 + 3 * 5 + lhsoffst] - coeff
				* lhss[0 + 3 * 5 + lhsoffst];
		lhss[2 + 4 * 5 + lhsoffst] = lhss[2 + 4 * 5 + lhsoffst] - coeff
				* lhss[0 + 4 * 5 + lhsoffst];
		c[2 + 0 * 5 + coffst] = c[2 + 0 * 5 + coffst] - coeff
				* c[0 + 0 * 5 + coffst];
		c[2 + 1 * 5 + coffst] = c[2 + 1 * 5 + coffst] - coeff
				* c[0 + 1 * 5 + coffst];
		c[2 + 2 * 5 + coffst] = c[2 + 2 * 5 + coffst] - coeff
				* c[0 + 2 * 5 + coffst];
		c[2 + 3 * 5 + coffst] = c[2 + 3 * 5 + coffst] - coeff
				* c[0 + 3 * 5 + coffst];
		c[2 + 4 * 5 + coffst] = c[2 + 4 * 5 + coffst] - coeff
				* c[0 + 4 * 5 + coffst];
		r[2 + roffst] = r[2 + roffst] - coeff * r[0 + roffst];

		coeff = lhss[3 + 0 * 5 + lhsoffst];
		lhss[3 + 1 * 5 + lhsoffst] = lhss[3 + 1 * 5 + lhsoffst] - coeff
				* lhss[0 + 1 * 5 + lhsoffst];
		lhss[3 + 2 * 5 + lhsoffst] = lhss[3 + 2 * 5 + lhsoffst] - coeff
				* lhss[0 + 2 * 5 + lhsoffst];
		lhss[3 + 3 * 5 + lhsoffst] = lhss[3 + 3 * 5 + lhsoffst] - coeff
				* lhss[0 + 3 * 5 + lhsoffst];
		lhss[3 + 4 * 5 + lhsoffst] = lhss[3 + 4 * 5 + lhsoffst] - coeff
				* lhss[0 + 4 * 5 + lhsoffst];
		c[3 + 0 * 5 + coffst] = c[3 + 0 * 5 + coffst] - coeff
				* c[0 + 0 * 5 + coffst];
		c[3 + 1 * 5 + coffst] = c[3 + 1 * 5 + coffst] - coeff
				* c[0 + 1 * 5 + coffst];
		c[3 + 2 * 5 + coffst] = c[3 + 2 * 5 + coffst] - coeff
				* c[0 + 2 * 5 + coffst];
		c[3 + 3 * 5 + coffst] = c[3 + 3 * 5 + coffst] - coeff
				* c[0 + 3 * 5 + coffst];
		c[3 + 4 * 5 + coffst] = c[3 + 4 * 5 + coffst] - coeff
				* c[0 + 4 * 5 + coffst];
		r[3 + roffst] = r[3 + roffst] - coeff * r[0 + roffst];

		coeff = lhss[4 + 0 * 5 + lhsoffst];
		lhss[4 + 1 * 5 + lhsoffst] = lhss[4 + 1 * 5 + lhsoffst] - coeff
				* lhss[0 + 1 * 5 + lhsoffst];
		lhss[4 + 2 * 5 + lhsoffst] = lhss[4 + 2 * 5 + lhsoffst] - coeff
				* lhss[0 + 2 * 5 + lhsoffst];
		lhss[4 + 3 * 5 + lhsoffst] = lhss[4 + 3 * 5 + lhsoffst] - coeff
				* lhss[0 + 3 * 5 + lhsoffst];
		lhss[4 + 4 * 5 + lhsoffst] = lhss[4 + 4 * 5 + lhsoffst] - coeff
				* lhss[0 + 4 * 5 + lhsoffst];
		c[4 + 0 * 5 + coffst] = c[4 + 0 * 5 + coffst] - coeff
				* c[0 + 0 * 5 + coffst];
		c[4 + 1 * 5 + coffst] = c[4 + 1 * 5 + coffst] - coeff
				* c[0 + 1 * 5 + coffst];
		c[4 + 2 * 5 + coffst] = c[4 + 2 * 5 + coffst] - coeff
				* c[0 + 2 * 5 + coffst];
		c[4 + 3 * 5 + coffst] = c[4 + 3 * 5 + coffst] - coeff
				* c[0 + 3 * 5 + coffst];
		c[4 + 4 * 5 + coffst] = c[4 + 4 * 5 + coffst] - coeff
				* c[0 + 4 * 5 + coffst];
		...
		lhss[0 + 2 * 5 + lhsoffst] = lhss[0 + 2 * 5 + lhsoffst] - coeff
				* lhss[1 + 2 * 5 + lhsoffst];
		lhss[0 + 3 * 5 + lhsoffst] = lhss[0 + 3 * 5 + lhsoffst] - coeff
				* lhss[1 + 3 * 5 + lhsoffst];
		lhss[0 + 4 * 5 + lhsoffst] = lhss[0 + 4 * 5 + lhsoffst] - coeff
				* lhss[1 + 4 * 5 + lhsoffst];
		c[0 + 0 * 5 + coffst] = c[0 + 0 * 5 + coffst] - coeff
				* c[1 + 0 * 5 + coffst];
		c[0 + 1 * 5 + coffst] = c[0 + 1 * 5 + coffst] - coeff
				* c[1 + 1 * 5 + coffst];
		c[0 + 2 * 5 + coffst] = c[0 + 2 * 5 + coffst] - coeff
				* c[1 + 2 * 5 + coffst];
		c[0 + 3 * 5 + coffst] = c[0 + 3 * 5 + coffst] - coeff
				* c[1 + 3 * 5 + coffst];
		c[0 + 4 * 5 + coffst] = c[0 + 4 * 5 + coffst] - coeff
				* c[1 + 4 * 5 + coffst];

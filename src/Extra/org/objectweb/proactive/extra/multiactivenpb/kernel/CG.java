...
public class CG extends CGBase implements RunActive {
	...
	public static void main(String argv[]) {
		...
		try {
			...
			activeInstance = PAActiveObject.turnActive(cg);
	...
	public void runBenchMark() {
		...
		makea(naa, nzz, a, colidx, rowstr, nonzer, rcond, arow, acol, aelt, v,
				iv, shift);
		...
		results = new BMResults(BMName, CLASS, na, 0, 0, niter, time,
				getMFLOPS(time, niter), "floating point", verified, serial,
				num_threads, bid);
		results.print();
		if (timeron)
			PrintTimers();
	...
	public double getMFLOPS(double total_time, int niter) {
		...
		if (total_time != 0.0) {
			mflops = (float) (2 * na)
					* (3. + (float) (nonzer * (nonzer + 1)) + 25.
							* (5. + (float) (nonzer * (nonzer + 1))) + 3.);
	...
	public int verify(double zeta) {
		...
		if (CLASS != 'U') {
			...
			if (Math.abs(zeta - zeta_verify_value) <= epsilon) {
				verified = 1;
				System.out.println(" Deviation is   "
	...
	public void makea(int n, int nz, double[] a, int[] colidx, int[] rowstr,
			int nonzer, double rcond, int[] arow, int[] acol, double[] aelt,
			double[] v, int[] iv, double shift) {
	...
	public void sparse(double a[], int colidx[], int rowstr[], int n,
	...
	public double conj_grad(int colidx[], int rowstr[], double x[], double z[],
	...
	private/* synchronized */void ExecuteTask(int OrderNum) {
		...
		for (int m = 0; m < num_threads; m++) {
			result.add(activeInstance.executeTask(m, OrderNum));
		}
	...
	@MemberOf("work")
	public IntWrapper executeTask(int m, int orderNum) {
		worker[m].runOnce(orderNum, alpha, beta);

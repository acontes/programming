...
@DefineGroups({ @Group(name = "runtime", selfCompatible = false),
		@Group(name = "interp", selfCompatible = true),
		@Group(name = "resid", selfCompatible = true),
		@Group(name = "psinv", selfCompatible = true),
		@Group(name = "rprj", selfCompatible = true) })
@DefineRules({ @Compatible(value = { "runtime", "interp" }),
		@Compatible(value = { "runtime", "resid" }),
		@Compatible(value = { "runtime", "psinv" }),
		@Compatible(value = { "runtime", "rprj" }) })
public class MG extends MGBase implements RunActive {
	
	static MG activeInstance;
	...
	public static void main(String argv[]) {
		...
		try {
			...
			activeInstance = PAActiveObject.turnActive(mg);
	...
	public boolean runBenchMark() {
		...
		results = new BMResults("MG", CLASS, nx[lt - 1], ny[lt - 1],
				nz[lt - 1], nit, tm, getMFLOPS(tm, nit), "floating point",
	...
	public int getInputPars() {
		...
		if (f2.exists()) {
			...
			try {
				...
				lt = datafile.readInt();
				if (lt > maxlevel) {
					System.out.println("lt=" + lt + " Maximum allowable="
							+ maxlevel);
					System.exit(0);
				}
			...
			System.out
					.println(" No input file mg.input, Using compiled defaults");
		}
		System.out.println(" Size:  " + nx[lt - 1] + "x" + ny[lt - 1] + "x"
				+ nz[lt - 1] + " Iterations:   " + nit);
		return nit;
	}
	...
	public void zran3(double z[], int n1, int n2, int n3, int nx, int ny) {
		...
		double temp, best;
		int i;
		int j1[] = new int[mm * 2], j2[] = new int[mm * 2], j3[] = new int[mm * 2];
		int jg[] = new int[4 * mm * 2], jg_temp[] = new int[4];
		...
		i0 = mm;
		for (i = mm - 1; i >= 0; i--) {
			best = z[j1[i1 - 1 + mm] + n1
					* (j2[i1 - 1 + mm] + n2 * (j3[i1 - 1 + mm]))];
			if (best == z[j1[i1 - 1 + mm] + n1
					* (j2[i1 - 1 + mm] + n2 * (j3[i1 - 1 + mm]))]) {
				jg[4 * (i + mm)] = 0;
	...
	public double norm2u3(double r[], int n1, int n2, int n3, double rnmu,
	...
	public void bubble(double ten[], int j1[], int j2[], int j3[], int m,
			int ind) {
		// c---------------------------------------------------------------------
	...
	public void resid(double u[], double v[], double r[], int off, int n1,
			int n2, int n3) {
		// c---------------------------------------------------------------------
		...
		for (i3 = 1; i3 < n3 - 1; i3++)
			for (i2 = 1; i2 < n2 - 1; i2++) {
				...
				for (i1 = 1; i1 < n1 - 1; i1++) {
					r[off + i1 + n1 * (i2 + n3 * i3)] = v[off + i1 + n1
							* (i2 + n3 * i3)]
							- a[0]
							* u[off + i1 + n1 * (i2 + n3 * i3)]
							// c---------------------------------------------------------------------
							// c Assume a(1) = 0 (Enable 2 lines below if a(1)
							// not= 0)
							// c---------------------------------------------------------------------
	...
	public void mg3Pmaster(double u[], double v[], double r[], int n1, int n2,
			int n3) {
		// c---------------------------------------------------------------------
		// c multigrid V-cycle routine
		// c---------------------------------------------------------------------
		...
		for (k = lt - 1; k >= lb; k--) {
			j = k - 1;
			rprj3Master(r, ir[k], m1[k], m2[k], m3[k], ir[j], m1[j], m2[j],
					m3[j]);
		}
		k = lb - 1;
		// c---------------------------------------------------------------------
		...
		for (k = lb; k < lt - 1; k++) {
			...
			// c---------------------------------------------------------------------
			zero3(u, ir[k], m1[k], m2[k], m3[k]);
			interpMaster(u, ir[j], m1[j], m2[j], m3[j], ir[k], m1[k], m2[k],
					m3[k]);
			// c---------------------------------------------------------------------
			// c compute residual for level k
			// c---------------------------------------------------------------------
			...
			for (j2 = 2; j2 <= m2j - 1; j2++) {
				...
				for (j1 = 2; j1 <= m1j - 1; j1++) {
					...
					x2 = r[roff + i1 + m1k * (i2 - 1 + m2k * i3)]
							...
							+ r[roff + i1 + m1k * (i2 + m2k * (i3 + 1))];
					r[soff + j1 - 1 + m1j * (j2 - 1 + m2j * (j3 - 1))] = 0.5
	...
	public void interp(double u[], int zoff, int mm1, int mm2, int mm3,
		...
		// c parameter( m=535 )
		int m = 535;
		double z1[] = new double[m], z2[] = new double[m], z3[] = new double[m];
		if (timeron)
	...
	public void psinv(double r[], int roff, double u[], int uoff, int n1,
			int n2, int n3) {
		// c---------------------------------------------------------------------
		...
		double r1[] = new double[nm + 1], r2[] = new double[nm + 1];
	...
	public void residMaster(double u[], double v[], double r[], int off,
		...
		if (num_threads == 1)
			...
			for (int m = 0; m < num_threads; m++) {
				result.add(activeInstance.doResid(m, n1, n2, n3, visr, off));
			}
	...
	@MemberOf("resid")
	public IntWrapper doResid(int l, int n1, int n2, int n3, boolean visr, int off) {
	...
	public void psinvMaster(double r[], int roffl, double u[], int uoffl,
			int n1, int n2, int n3) {
		if (timeron)
			timer.start(T_psinv);
		if (num_threads == 1)
			...
			LinkedList<IntWrapper> result = new LinkedList<IntWrapper>();
			for (int m = 0; m < num_threads; m++) {
				result.add(activeInstance.doPsinv(m, n1, n2, n3, roffl, uoffl));
			}
	...
	@MemberOf("psinv")
	public IntWrapper doPsinv(int l, int n1, int n2, int n3, int roffl, int uoffl) {
	...
	public void interpMaster(double u[], int zoffl, int mm1, int mm2, int mm3,
		...
		if (num_threads == 1)
			...
			LinkedList<IntWrapper> result = new LinkedList<IntWrapper>();
			for (int m = 0; m < num_threads; m++) {
				result.add(activeInstance.doInterp(m, n1, n2, n3, mm1, mm2, mm3, zoffl, uoffl));
			}
	...
	@MemberOf("interp")
	public IntWrapper doInterp(int l, int n1, int n2, int n3, int mm1, int mm2, int mm3, int zoffl, int uoffl) {
		interp[l].done = false;
		interp[l].wstart = 1;
		interp[l].wend = mm3;
	...
	public void rprj3Master(double r[], int roffl, int m1k, int m2k, int m3k,
		...
		if (num_threads == 1)
			...
			LinkedList<IntWrapper> result = new LinkedList<IntWrapper>();
			for (int m = 0; m < num_threads; m++) {
				result.add(activeInstance.doRprj(m, m1k, m2k, m3k, m1j, m2j, m3j, roffl, soffl));
			}
	...
	@MemberOf("rprj")
	public IntWrapper doRprj(int l, int m1k, int m2k, int m3k, int m1j, int m2j, int m3j, int roffl, int soffl) {
		rprj[l].done = false;
		rprj[l].wstart = 2;
		rprj[l].wend = m3j;
		rprj[l].m1k = m1k;

...
import org.objectweb.proactive.extra.multiactivenpb.ft.FTBase;
import org.objectweb.proactive.multiactivity.MultiActiveService;

@DefineGroups( {
	@Group(name="runtime", selfCompatible=false),
	@Group(name="fft", selfCompatible=true),
	@Group(name="evolve", selfCompatible=true)
})
@DefineRules({
	@Compatible(value={"runtime", "fft"}),
	@Compatible(value={"runtime", "evolve"})
})
public class FT extends FTBase implements RunActive {
	
	static FT activeInstance;
	...
	public static void main(String argv[]) {
		...
		try {
			...
			activeInstance = PAActiveObject.turnActive(ft);
	...
	public void runBenchMark() {
		...
		results = new BMResults(BMName, CLASS, nx, ny, nz, niter_default, time,
				getMFLOPS(time, nx, ny, nz), "floating point", verified,
				serial, num_threads, bid);
		results.print();
		if (timeron)
			printTimers();
		done = true;
		...
		for (int it = 0; it < niter_default; it++) {
			...
			for (int i = 0; i < nx; i++) {
				...
				for (int k = 0; k < nz; k++) {
					...
					for (int j = 0; j < ny; j++) {
						int jj = j - ((j) / n22) * ny;
						xnt[REAL + j * isize4 + k * jsize4 + i * ksize4] = xtr[REAL
								+ j * isize3 + i * jsize3 + k * ksize3]
								* Math.exp((ap * (jj * jj + ik2)) * (it + 1));
						xnt[IMAG + j * isize4 + k * jsize4 + i * ksize4] = xtr[IMAG
								+ j * isize3 + i * jsize3 + k * ksize3]
								* Math.exp((ap * (jj * jj + ik2)) * (it + 1));
					}
				}
	...
	public /*synchronized */ void doFFT() {
		...
		for (int m = 0; m < num_threads; m++) {
			result.add(activeInstance.doFFT(m));
		}
	...
	@MemberOf("fft")
	public IntWrapper doFFT(int n) {
		doFFT[n].runOnce();
		return new IntWrapper(1);
	}
	
	@MemberOf("evolve")
	public IntWrapper doEvolve(int n, int it) {
		doEvolve[n].runOnce(it);
		return new IntWrapper(1);
	}
...
/*		int m;
		...
		LinkedList<IntWrapper> result = new LinkedList<IntWrapper>();
		for (int m = 0; m < num_threads; m++) {
			result.add(activeInstance.doEvolve(m, it));
		}
	...
	public void printTimers() {
		...
		System.out
				.println("FT time =		      " + fmt.format(timer.readTimer(1)));
		System.out.println("WarmUp time =		      "
				+ fmt.format(timer.readTimer(2)));
		System.out.println("ffXYZ body time =	      "
				+ fmt.format(timer.readTimer(3)));
		System.out.println("Swarztrauber body time =      "
				+ fmt.format(timer.readTimer(4)));
		System.out.println("Redistribution time =	      "
				+ fmt.format(timer.readTimer(5)));
		System.out.println("Transposition time =	      "
				+ fmt.format(timer.readTimer(6)));
		...
		System.out.println("CalculateChecksum =	      "
				+ fmt.format(timer.readTimer(10)));
		System.out
				.println("evolve =		      " + fmt.format(timer.readTimer(11)));
		System.out.println("compute_initial_conditions =  "
				+ fmt.format(timer.readTimer(12)));
		System.out.println("twiddle =		      "
				+ fmt.format(timer.readTimer(13)));
		System.out
				.println("verify =		      " + fmt.format(timer.readTimer(14)));
		System.out
				.println("fftXYZ =		      " + fmt.format(timer.readTimer(15)));
	}
	...
	public void CalculateChecksum(double csum[], int csmffst, int iterN,
			double u[], int d1, int d2, int d3) {
		int i, ii, ji, ki;
		int isize3 = 2, jsize3 = isize3 * (d1 + 1), ksize3 = jsize3 * d2;
	...
	public void fftXYZ(int sign, double x[], double exp1[], double exp2[],
		...
		for (k = 0; k < n3; k++) {
			for (j = 0; j < n2; j++) {
				for (i = 0; i < n1; i++) {
					plane[REAL + j * isize1 + i * jsize1] = x[REAL + i * isize3
							+ j * jsize3 + k * ksize3];
					plane[IMAG + j * isize1 + i * jsize1] = x[IMAG + i * isize3
							+ j * jsize3 + k * ksize3];
				}
			}
		...
		for (k = 0; k < n2; k++) {
			for (i = 0; i < n3; i++) {
				for (j = 0; j < n1; j++) {
					plane[REAL + j * isize1 + i * jsize1] = x[REAL + j * isize3
							+ k * jsize3 + i * ksize3];
					plane[IMAG + j * isize1 + i * jsize1] = x[IMAG + j * isize3
							+ k * jsize3 + i * ksize3];
				}
			}
	...
	public int verify(int ires, int n1, int n2, int n3, int nt, double cksum[]) {
		...
		if (nt <= 0) {
			...
			for (int it = 0; it < nt; it++) {
				double csumr = (cksum[REAL + it * 2] - cexpd[REAL + it * 2])
						/ cexpd[REAL + it * 2];
				double csumi = (cksum[IMAG + it * 2] - cexpd[IMAG + it * 2])
						/ cexpd[IMAG + it * 2];
				if (Math.abs(csumr) <= epsilon || Math.abs(csumi) <= epsilon) {
					if (verified == -1)
						verified = 1;
				} else {
					verified = 0;

...
import org.objectweb.proactive.extra.multiactivenpb.sp.*;
...
@DefineGroups({ @Group(name = "runtime", selfCompatible = false),
		...
		@Group(name = "compute", selfCompatible = true),
		@Group(name = "inverse", selfCompatible = true),
		@Group(name = "xsolver", selfCompatible = true),
		@Group(name = "ysolver", selfCompatible = true),
		@Group(name = "zsolver", selfCompatible = true) })
@DefineRules({ @Compatible(value = { "runtime", "adder" }),
		@Compatible(value = { "runtime", "compute" }),
		@Compatible(value = { "runtime", "inverse" }),
		@Compatible(value = { "runtime", "xsolver" }),
		@Compatible(value = { "runtime", "ysolver" }),
		@Compatible(value = { "runtime", "zsolver" }) })
public class SP extends SPBase implements RunActive{
	
	static SP activeInstance;
	...
	public static void main(String argv[]) {
		...
		try {
			...
			activeInstance = PAActiveObject.turnActive(sp);
	...
	void doInverse(){
		...
		for (int m = 0; m < num_threads; m++) {
			result.add(activeInstance.doInverse(m));
		}
	...
	@MemberOf("inverse")
	public IntWrapper doInverse(int m) {
		txinverse[m].runOnce();
	...
	@MemberOf("adder")
	public IntWrapper doRHSAdd(int m) {
		rhsadder[m].runOnce();
	...
	/*synchronized*/ void doRHS() {
		...
		LinkedList<IntWrapper> result = new LinkedList<IntWrapper>();
		for (int m = 0; m < num_threads; m++) {
			result.add(activeInstance.doRHS(m));
		}
...
/*		int m;
		...
		for (int i = 1; i < t_last; i++) {
			...
			if (i == t_rhs) {
				...
				System.out.println("    --> total " + "sub-rhs" + ":"
						+ fmt.format(t) + "  (" + fmt.format(t * 100. / tmax)
						+ "%)");
				t = trecs[t_rhs] - t;
				...
				System.out.println("    --> total " + "sub-zsol" + ":"
						+ fmt.format(t) + "  (" + fmt.format(t * 100. / tmax)
						+ "%)");
			} else if (i == t_rdis2) {
				t = trecs[t_rdis1] + trecs[t_rdis2];
				System.out.println("    --> total " + "redist" + ":"
						+ fmt.format(t) + "  (" + fmt.format(t * 100. / tmax)
						+ "%)");
			}
		}
	...
	public void add() {
		...
		for (k = 1; k <= nz2; k++) {
			for (j = 1; j <= ny2; j++) {
				for (i = 1; i <= nx2; i++) {
					for (m = 0; m <= 4; m++) {
						u[m + i * isize1 + j * jsize1 + k * ksize1] += rhs[m
		...
		for (k = 0; k <= grid_points[2] - 1; k++) {
			...
			for (j = 0; j <= grid_points[1] - 1; j++) {
				...
				for (i = 0; i <= grid_points[0] - 1; i++) {
					...
					for (m = 0; m <= 4; m++) {
						add = u[m + i * isize1 + j * jsize1 + k * ksize1]
								- u_exact[m];
						rms[m] = rms[m] + add * add;
					}
				}
	...
	public void exact_rhs() {
		...
		for (k = 1; k <= grid_points[2] - 2; k++) {
			...
			for (j = 1; j <= grid_points[1] - 2; j++) {
				...
				for (i = 1; i <= grid_points[0] - 2; i++) {
					...
					forcing[0 + i * isize1 + j * jsize1 + k * ksize1] = forcing[0
							+ i * isize1 + j * jsize1 + k * ksize1]
							- tx2
					...
					forcing[1 + i * isize1 + j * jsize1 + k * ksize1] = forcing[1
							+ i * isize1 + j * jsize1 + k * ksize1]
							- tx2
					...
					forcing[2 + i * isize1 + j * jsize1 + k * ksize1] = forcing[2
							+ i * isize1 + j * jsize1 + k * ksize1]
							- tx2
					...
					forcing[3 + i * isize1 + j * jsize1 + k * ksize1] = forcing[3
							+ i * isize1 + j * jsize1 + k * ksize1]
							- tx2
					...
					forcing[4 + i * isize1 + j * jsize1 + k * ksize1] = forcing[4
							+ i * isize1 + j * jsize1 + k * ksize1]
							- tx2
							* (buf[ip1 + 1 * jsize3]
				...
				for (m = 0; m <= 4; m++) {
					i = 1;
					forcing[m + i * isize1 + j * jsize1 + k * ksize1] = forcing[m
							+ i * isize1 + j * jsize1 + k * ksize1]
							- dssp
		...
		for (k = 1; k <= grid_points[2] - 2; k++) {
			for (j = 1; j <= grid_points[1] - 2; j++) {
				for (i = 1; i <= grid_points[0] - 2; i++) {
					for (m = 0; m <= 4; m++) {
						forcing[m + i * isize1 + j * jsize1 + k * ksize1] = -1.
								* forcing[m + i * isize1 + j * jsize1 + k
										* ksize1];
					}
				}
			}
		...
		for (k = 1; k <= nz2; k++) {
			for (j = 1; j <= ny2; j++) {
				for (i = 1; i <= nx2; i++) {
					...
					rhs[2 + i * isize1 + j * jsize1 + k * ksize1] = bt
							* (r4 - r5);
		...
		for (k = 1; k <= nz2; k++) {
			for (j = 1; j <= ny2; j++) {
				for (i = 1; i <= nx2; i++) {
					...
					rhs[0 + i * isize1 + j * jsize1 + k * ksize1] = bt
							* (r4 - r5);
	...
	public void compute_rhs() {
		...
		for (k = 0; k <= grid_points[2] - 1; k++) {
			for (j = 0; j <= grid_points[1] - 1; j++) {
				for (i = 0; i <= grid_points[0] - 1; i++) {
					...
					us[i + j * jsize2 + k * ksize2] = u[1 + i * isize1 + j
							* jsize1 + k * ksize1]
							* rho_inv;
					vs[i + j * jsize2 + k * ksize2] = u[2 + i * isize1 + j
							* jsize1 + k * ksize1]
							* rho_inv;
					ws[i + j * jsize2 + k * ksize2] = u[3 + i * isize1 + j
							* jsize1 + k * ksize1]
							* rho_inv;
					square[i + j * jsize2 + k * ksize2] = 0.5
							* (u[1 + i * isize1 + j * jsize1 + k * ksize1]
									...
									* u[3 + i * isize1 + j * jsize1 + k
											* ksize1]) * rho_inv;
					qs[i + j * jsize2 + k * ksize2] = square[i + j * jsize2 + k
							* ksize2]
							* rho_inv;
					// ---------------------------------------------------------------------
		...
		for (k = 0; k <= grid_points[2] - 1; k++) {
			for (j = 0; j <= grid_points[1] - 1; j++) {
				for (i = 0; i <= grid_points[0] - 1; i++) {
					for (m = 0; m <= 4; m++) {
						rhs[m + i * isize1 + j * jsize1 + k * ksize1] = forcing[m
								+ i * isize1 + j * jsize1 + k * ksize1];
					}
				}
			}
		...
		for (k = 1; k <= nz2; k++) {
			for (j = 1; j <= ny2; j++) {
					...
					rhs[0 + i * isize1 + j * jsize1 + k * ksize1] = rhs[0 + i
							...
							* (u[0 + (i + 1) * isize1 + j * jsize1 + k * ksize1]
									...
									* u[0 + i * isize1 + j * jsize1 + k
									...
									+ j * jsize1 + k * ksize1])
							- tx2
							* (u[1 + (i + 1) * isize1 + j * jsize1 + k * ksize1] - u[1
									+ (i - 1)
									* isize1
									+ j
									* jsize1
					...
					rhs[2 + i * isize1 + j * jsize1 + k * ksize1] = rhs[2 + i
							...
							* (u[2 + (i + 1) * isize1 + j * jsize1 + k * ksize1]
									...
									* u[2 + i * isize1 + j * jsize1 + k
									...
									+ j * jsize1 + k * ksize1])
							+ xxcon2
							* (vs[i + 1 + j * jsize2 + k * ksize2] - 2.0
									* vs[i + j * jsize2 + k * ksize2] + vs[i
									- 1 + j * jsize2 + k * ksize2])
							- tx2
							* (u[2 + (i + 1) * isize1 + j * jsize1 + k * ksize1]
									* up1 - u[2 + (i - 1) * isize1 + j * jsize1
									+ k * ksize1]
									* um1);
					...
					rhs[3 + i * isize1 + j * jsize1 + k * ksize1] = rhs[3 + i
							...
							* (u[3 + (i + 1) * isize1 + j * jsize1 + k * ksize1]
									...
									* u[3 + i * isize1 + j * jsize1 + k
									...
									+ j * jsize1 + k * ksize1])
							+ xxcon2
							* (ws[i + 1 + j * jsize2 + k * ksize2] - 2.0
									* ws[i + j * jsize2 + k * ksize2] + ws[i
									- 1 + j * jsize2 + k * ksize2])
							- tx2
							* (u[3 + (i + 1) * isize1 + j * jsize1 + k * ksize1]
									* up1 - u[3 + (i - 1) * isize1 + j * jsize1
									+ k * ksize1]
									* um1);
					...
					rhs[4 + i * isize1 + j * jsize1 + k * ksize1] = rhs[4 + i
							...
							* (u[4 + (i + 1) * isize1 + j * jsize1 + k * ksize1]
									...
									* u[4 + i * isize1 + j * jsize1 + k
									...
									+ j * jsize1 + k * ksize1])
							+ xxcon3
							* (qs[i + 1 + j * jsize2 + k * ksize2] - 2.0
									* qs[i + j * jsize2 + k * ksize2] + qs[i
									- 1 + j * jsize2 + k * ksize2])
							+ xxcon4
							...
							* (u[4 + (i + 1) * isize1 + j * jsize1 + k * ksize1]
									...
									* u[4 + i * isize1 + j * jsize1 + k
											* ksize1]
									* rho_i[i + j * jsize2 + k * ksize2] + u[4
									+ (i - 1) * isize1 + j * jsize1 + k
									* ksize1]
									* rho_i[i - 1 + j * jsize2 + k * ksize2])
							- tx2
							* ((c1
				...
				i = 1;
				for (m = 0; m <= 4; m++) {
					rhs[m + i * isize1 + j * jsize1 + k * ksize1] = rhs[m + i
							* isize1 + j * jsize1 + k * ksize1]
							- dssp
							* (5.0
									* u[m + i * isize1 + j * jsize1 + k
											* ksize1]
									- 4.0
									* u[m + (i + 1) * isize1 + j * jsize1 + k
											* ksize1] + u[m + (i + 2) * isize1
									+ j * jsize1 + k * ksize1]);
				}
				...
				for (i = 3; i <= nx2 - 2; i++) {
					for (m = 0; m <= 4; m++) {
						rhs[m + i * isize1 + j * jsize1 + k * ksize1] = rhs[m
								+ i * isize1 + j * jsize1 + k * ksize1]
								- dssp
					...
					rhs[0 + i * isize1 + j * jsize1 + k * ksize1] = rhs[0 + i
							...
							* (u[0 + i * isize1 + (j + 1) * jsize1 + k * ksize1]
									...
									* u[0 + i * isize1 + j * jsize1 + k
									...
									+ (j - 1) * jsize1 + k * ksize1])
							- ty2
							* (u[2 + i * isize1 + (j + 1) * jsize1 + k * ksize1] - u[2
									+ i
									* isize1
									+ (j - 1)
									* jsize1
					...
					rhs[1 + i * isize1 + j * jsize1 + k * ksize1] = rhs[1 + i
							...
							* (u[1 + i * isize1 + (j + 1) * jsize1 + k * ksize1]
									...
									* u[1 + i * isize1 + j * jsize1 + k
									...
									+ (j - 1) * jsize1 + k * ksize1])
							+ yycon2
							* (us[i + (j + 1) * jsize2 + k * ksize2] - 2.0
									* us[i + j * jsize2 + k * ksize2] + us[i
									+ (j - 1) * jsize2 + k * ksize2])
							- ty2
							* (u[1 + i * isize1 + (j + 1) * jsize1 + k * ksize1]
									* vp1 - u[1 + i * isize1 + (j - 1) * jsize1
									+ k * ksize1]
									* vm1);
					rhs[2 + i * isize1 + j * jsize1 + k * ksize1] = rhs[2 + i
							* isize1 + j * jsize1 + k * ksize1]
							...
							* (u[2 + i * isize1 + (j + 1) * jsize1 + k * ksize1]
									* vp1
									- u[2 + i * isize1 + (j - 1) * jsize1 + k
											* ksize1] * vm1 + (u[4 + i * isize1
									+ (j + 1) * jsize1 + k * ksize1]
									- square[i + (j + 1) * jsize2 + k * ksize2]
					...
					rhs[3 + i * isize1 + j * jsize1 + k * ksize1] = rhs[3 + i
							...
							* (u[3 + i * isize1 + (j + 1) * jsize1 + k * ksize1]
									...
									* u[3 + i * isize1 + j * jsize1 + k
									...
									+ (j - 1) * jsize1 + k * ksize1])
							+ yycon2
							* (ws[i + (j + 1) * jsize2 + k * ksize2] - 2.0
									* ws[i + j * jsize2 + k * ksize2] + ws[i
									+ (j - 1) * jsize2 + k * ksize2])
							- ty2
							* (u[3 + i * isize1 + (j + 1) * jsize1 + k * ksize1]
									* vp1 - u[3 + i * isize1 + (j - 1) * jsize1
									+ k * ksize1]
									* vm1);
					rhs[4 + i * isize1 + j * jsize1 + k * ksize1] = rhs[4 + i
							* isize1 + j * jsize1 + k * ksize1]
		...
		for (k = 1; k <= nz2; k++) {
			for (j = 1; j <= ny2; j++) {
				for (i = 1; i <= nx2; i++) {
					...
					t1 = c2
							* ac2inv
							* (qs[i + j * jsize2 + k * ksize2] * r1 - uu * r2
									- vv * r3 - ww * r4 + r5);
					t2 = bt * ru1 * (uu * r1 - r2);
					t3 = (bt * ru1 * ac) * t1;
	...
	public int verify(int no_time_steps) {
		...
		if (grid_points[0] == 12 && grid_points[1] == 12
			...
			// ---------------------------------------------------------------------
			// reference data for 64X64X64 grids after 400 time steps, with DT =
			// 1.5d-03
			// ---------------------------------------------------------------------
		...
		for (k = 1; k <= nz2; k++) {
			for (j = 1; j <= ny2; j++) {
				...
				for (i = 0; i <= grid_points[0] - 1; i++) {
					...
					rhon[i] = dmax1(dx2 + con43 * ru1, dx5 + c1c5 * ru1, dxmax
							+ ru1, dx1);
				}
				...
				for (i = 1; i <= nx2; i++) {
					lhs[0 + i * jsize4] = 0.0;
					lhs[1 + i * jsize4] = -dttx2 * cv[(i - 1)] - dttx1
							* rhon[i - 1];
					lhs[2 + i * jsize4] = 1.0 + c2dttx1 * rhon[i];
					lhs[3 + i * jsize4] = dttx2 * cv[i + 1] - dttx1
							* rhon[i + 1];
					lhs[4 + i * jsize4] = 0.0;
				}
				...
				// ---------------------------------------------------------------------
				// subsequently, fill the other factors (u+c), (u-c) by adding
				// to
				// the first
				// ---------------------------------------------------------------------
				for (i = 1; i <= nx2; i++) {
					lhsp[0 + i * jsize4] = lhs[0 + i * jsize4];
					lhsp[1 + i * jsize4] = lhs[1 + i * jsize4] - dttx2
							* speed[(i - 1) + j * jsize2 + k * ksize2];
					lhsp[2 + i * jsize4] = lhs[2 + i * jsize4];
					lhsp[3 + i * jsize4] = lhs[3 + i * jsize4] + dttx2
							* speed[i + 1 + j * jsize2 + k * ksize2];
					lhsp[4 + i * jsize4] = lhs[4 + i * jsize4];
					lhsm[0 + i * jsize4] = lhs[0 + i * jsize4];
					lhsm[1 + i * jsize4] = lhs[1 + i * jsize4] + dttx2
							* speed[i - 1 + j * jsize2 + k * ksize2];
					lhsm[2 + i * jsize4] = lhs[2 + i * jsize4];
					lhsm[3 + i * jsize4] = lhs[3 + i * jsize4] - dttx2
							* speed[i + 1 + j * jsize2 + k * ksize2];
				...
				for (i = 0; i <= grid_points[0] - 3; i++) {
					...
					for (m = 0; m <= 2; m++) {
						rhs[m + i * isize1 + j * jsize1 + k * ksize1] = fac1
		...
		for (k = 1; k <= grid_points[2] - 2; k++) {
			for (i = 1; i <= grid_points[0] - 2; i++) {
				...
				for (j = 0; j <= grid_points[1] - 1; j++) {
					...
					rhoq[j] = dmax1(dy3 + con43 * ru1, dy5 + c1c5 * ru1, dymax
							+ ru1, dy1);
				}
				...
				for (j = 1; j <= grid_points[1] - 2; j++) {
					lhs[0 + j * jsize4] = 0.0;
					lhs[1 + j * jsize4] = -dtty2 * cv[j - 1] - dtty1
							* rhoq[j - 1];
					lhs[2 + j * jsize4] = 1.0 + c2dtty1 * rhoq[j];
					lhs[3 + j * jsize4] = dtty2 * cv[j + 1] - dtty1
							* rhoq[j + 1];
					lhs[4 + j * jsize4] = 0.0;
				}
				...
				for (j = 1; j <= grid_points[1] - 2; j++) {
					...
					lhsp[1 + j * jsize4] = lhs[1 + j * jsize4] - dtty2
							* speed[i + (j - 1) * jsize2 + k * ksize2];
					lhsp[2 + j * jsize4] = lhs[2 + j * jsize4];
					lhsp[3 + j * jsize4] = lhs[3 + j * jsize4] + dtty2
							* speed[i + (j + 1) * jsize2 + k * ksize2];
					lhsp[4 + j * jsize4] = lhs[4 + j * jsize4];

					lhsm[0 + j * jsize4] = lhs[0 + j * jsize4];
					lhsm[1 + j * jsize4] = lhs[1 + j * jsize4] + dtty2
							* speed[i + (j - 1) * jsize2 + k * ksize2];
					lhsm[2 + j * jsize4] = lhs[2 + j * jsize4];
					lhsm[3 + j * jsize4] = lhs[3 + j * jsize4] - dtty2
							* speed[i + (j + 1) * jsize2 + k * ksize2];
		...
		for (j = 1; j <= ny2; j++) {
			for (i = 1; i <= nx2; i++) {
				...
				for (k = 0; k <= nz2 + 1; k++) {
					...
					rhos[k] = dmax1(dz4 + con43 * ru1, dz5 + c1c5 * ru1, dzmax
							+ ru1, dz1);
				}
				...
				for (k = 1; k <= nz2; k++) {
					lhs[0 + k * jsize4] = 0.0;
					lhs[1 + k * jsize4] = -dttz2 * cv[k - 1] - dttz1
							* rhos[k - 1];
					lhs[2 + k * jsize4] = 1.0 + c2dttz1 * rhos[k];
					lhs[3 + k * jsize4] = dttz2 * cv[k + 1] - dttz1
							* rhos[k + 1];
					lhs[4 + k * jsize4] = 0.0;
				}
				...
				for (k = 1; k <= nz2; k++) {
					...
					lhsp[1 + k * jsize4] = lhs[1 + k * jsize4] - dttz2
							* speed[i + j * jsize2 + (k - 1) * ksize2];
					lhsp[2 + k * jsize4] = lhs[2 + k * jsize4];
					lhsp[3 + k * jsize4] = lhs[3 + k * jsize4] + dttz2
							* speed[i + j * jsize2 + (k + 1) * ksize2];
					lhsp[4 + k * jsize4] = lhs[4 + k * jsize4];
					lhsm[0 + k * jsize4] = lhs[0 + k * jsize4];
					lhsm[1 + k * jsize4] = lhs[1 + k * jsize4] + dttz2
							* speed[i + j * jsize2 + (k - 1) * ksize2];
					lhsm[2 + k * jsize4] = lhs[2 + k * jsize4];
					lhsm[3 + k * jsize4] = lhs[3 + k * jsize4] - dttz2
							* speed[i + j * jsize2 + (k + 1) * ksize2];
				...
				for (k = 0; k <= nz2 + 1; k++) {
					rtmp[0 + k * 5] = rhs[0 + i * isize1 + j * jsize1 + k
							* ksize1];
					rtmp[1 + k * 5] = rhs[1 + i * isize1 + j * jsize1 + k
							* ksize1];
					rtmp[2 + k * 5] = rhs[2 + i * isize1 + j * jsize1 + k
							* ksize1];
					rtmp[3 + k * 5] = rhs[3 + i * isize1 + j * jsize1 + k
							* ksize1];
					rtmp[4 + k * 5] = rhs[4 + i * isize1 + j * jsize1 + k
							* ksize1];
				}
				...
				lhs[3 + k1 * jsize4] = lhs[3 + k1 * jsize4]
						- lhs[1 + k1 * jsize4] * lhs[4 + k * jsize4];
				for (m = 0; m <= 2; m++) {
					rtmp[m + k1 * 5] = rtmp[m + k1 * 5] - lhs[1 + k1 * jsize4]
							* rtmp[m + k * 5];
				}
				// ---------------------------------------------------------------------
				// scale the last row immediately
				...
				for (k = 0; k <= grid_points[2] - 3; k++) {
					...
					rtmp[m + k1 * 5] = rtmp[m + k1 * 5] - lhsp[1 + k1 * jsize4]
							* rtmp[m + k * 5];
					...
					rtmp[m + k2 * 5] = rtmp[m + k2 * 5] - lhsp[0 + k2 * jsize4]
							* rtmp[m + k * 5];
					m = 4;
					...
					rtmp[m + k1 * 5] = rtmp[m + k1 * 5] - lhsm[1 + k1 * jsize4]
							* rtmp[m + k * 5];
					...
					rtmp[m + k2 * 5] = rtmp[m + k2 * 5] - lhsm[0 + k2 * jsize4]
							* rtmp[m + k * 5];
				}
				...
				k1 = grid_points[2] - 1;
				for (m = 0; m <= 2; m++) {
					rtmp[m + k * 5] = rtmp[m + k * 5] - lhs[3 + k * jsize4]
							* rtmp[m + k1 * 5];
				}

				rtmp[3 + k * 5] = rtmp[3 + k * 5] - lhsp[3 + k * jsize4]
						* rtmp[3 + k1 * 5];
				rtmp[4 + k * 5] = rtmp[4 + k * 5] - lhsm[3 + k * jsize4]
						* rtmp[4 + k1 * 5];
				...
				for (k = grid_points[2] - 3; k >= 0; k--) {
					...
					// ---------------------------------------------------------------------
					rtmp[3 + k * 5] = rtmp[3 + k * 5] - lhsp[3 + k * jsize4]
							* rtmp[3 + k1 * 5] - lhsp[4 + k * jsize4]
							* rtmp[3 + k2 * 5];
					rtmp[4 + k * 5] = rtmp[4 + k * 5] - lhsm[3 + k * jsize4]
							* rtmp[4 + k1 * 5] - lhsm[4 + k * jsize4]
							* rtmp[4 + k2 * 5];
				}

...
public class ISBase extends Thread {
	...
	public static int S_test_index_array[] = { 48427, 17148, 23627, 62548, 4431 },
			...
			C_test_index_array[] = { 44172927, 72999161, 74326391, 129606274,
					21736814 }, C_test_rank_array[] = { 61147, 882988, 266290,
					133997595, 133525895 };
	...
	public void setupThreads(IS is) {
		...
		for (int i = 0; i < num_threads; i++) {
			...
			end = i * (TOTAL_KEYS / num_threads) + (TOTAL_KEYS / num_threads)
					- 1 + offset;
			if (remainder > 0) {
				remainder--;
				offset++;
			...
			rend = i * (MAX_KEY / num_threads) + (MAX_KEY / num_threads) - 1
					+ roffset;
			if (rremainder > 0) {
				rremainder--;
				roffset++;

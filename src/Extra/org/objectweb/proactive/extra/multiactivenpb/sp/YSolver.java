			...
			for (k = lower_bound; k <= upper_bound; k++) {
				for (i = 1; i <= grid_points[0] - 2; i++) {
					...
					for (j = 0; j <= grid_points[1] - 1; j++) {
						...
						rhoq[j] = dmax1(dy3 + con43 * ru1, dy5 + c1c5 * ru1,
								dymax + ru1, dy1);
					}
					...
					lhs[1 + (j + 1) * jsize4] = lhs[1 + (j + 1) * jsize4]
							- comz4;
					lhs[2 + (j + 1) * jsize4] = lhs[2 + (j + 1) * jsize4]
							+ comz6;
					lhs[3 + (j + 1) * jsize4] = lhs[3 + (j + 1) * jsize4]
							- comz4;
					lhs[4 + (j + 1) * jsize4] = lhs[4 + (j + 1) * jsize4]
							+ comz1;
					...
					lhs[0 + (j + 1) * jsize4] = lhs[0 + (j + 1) * jsize4]
							+ comz1;
					lhs[1 + (j + 1) * jsize4] = lhs[1 + (j + 1) * jsize4]
							- comz4;
					lhs[2 + (j + 1) * jsize4] = lhs[2 + (j + 1) * jsize4]
					...
					for (j = grid_points[1] - 3; j >= 0; j--) {
						...
						for (m = 0; m <= 2; m++) {
							rhs[m + i * isize1 + j * jsize1 + k * ksize1] = rhs[m
									+ i * isize1 + j * jsize1 + k * ksize1]
									- lhs[3 + j * jsize4]
									* rhs[m + i * isize1 + j1 * jsize1 + k
											* ksize1]
									- lhs[4 + j * jsize4]
									* rhs[m + i * isize1 + j2 * jsize1 + k
											* ksize1];
						}

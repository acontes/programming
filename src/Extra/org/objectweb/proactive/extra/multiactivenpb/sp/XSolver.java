			...
			for (k = lower_bound; k <= upper_bound; k++) {
				for (j = 1; j <= ny2; j++) {
					...
					for (i = 0; i <= grid_points[0] - 1; i++) {
						...
						rhon[i] = dmax1(dx2 + con43 * ru1, dx5 + c1c5 * ru1,
								dxmax + ru1, dx1);
					}
					...
					lhs[1 + (i + 1) * jsize4] = lhs[1 + (i + 1) * jsize4]
							- comz4;
					lhs[2 + (i + 1) * jsize4] = lhs[2 + (i + 1) * jsize4]
							+ comz6;
					lhs[3 + (i + 1) * jsize4] = lhs[3 + (i + 1) * jsize4]
							- comz4;
					lhs[4 + (i + 1) * jsize4] = lhs[4 + (i + 1) * jsize4]
							+ comz1;
					...
					lhs[0 + (i + 1) * jsize4] = lhs[0 + (i + 1) * jsize4]
							+ comz1;
					lhs[1 + (i + 1) * jsize4] = lhs[1 + (i + 1) * jsize4]
							- comz4;
					lhs[2 + (i + 1) * jsize4] = lhs[2 + (i + 1) * jsize4]
					...
					// ---------------------------------------------------------------------
					// subsequently, fill the other factors (u+c), (u-c) by
					// adding to
					// the first
					// ---------------------------------------------------------------------
					for (i = 1; i <= nx2; i++) {
					...
					for (i = grid_points[0] - 3; i >= 0; i--) {
						...
						for (m = 0; m <= 2; m++) {
							rhs[m + i * isize1 + j * jsize1 + k * ksize1] = rhs[m
									+ i * isize1 + j * jsize1 + k * ksize1]
									- lhs[3 + i * jsize4]
									* rhs[m + i1 * isize1 + j * jsize1 + k
											* ksize1]
									- lhs[4 + i * jsize4]
									* rhs[m + i2 * isize1 + j * jsize1 + k
											* ksize1];
						}
						// ---------------------------------------------------------------------
				...
				for (j = 1; j <= ny2; j++) {
					for (i = 1; i <= nx2; i++) {
						...
						rhs[2 + i * isize1 + j * jsize1 + k * ksize1] = bt
								* (r4 - r5);
						rhs[3 + i * isize1 + j * jsize1 + k * ksize1] = -t1
								+ t2;

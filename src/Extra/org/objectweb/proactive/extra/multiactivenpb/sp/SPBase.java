...
public class SPBase extends Thread {
	...
	protected int IMAX = 0, JMAX = 0, KMAX = 0, problem_size = 0, nx2 = 0,
			ny2 = 0, nz2 = 0;
	...
	protected double cv[], rhon[], rhos[], rhoq[], cuf[], q[];

	protected static double tx1, tx2, tx3, ty1, ty2, ty3, tz1, tz2, tz3, dx1,
			dx2, dx3, dx4, dx5, dy1, dy2, dy3, dy4, dy5, dz1, dz2, dz3, dz4,
			dz5, dssp, dt, dxmax, dymax, dzmax, xxcon1, xxcon2, xxcon3, xxcon4,
			xxcon5, dx1tx1, dx2tx1, dx3tx1, dx4tx1, dx5tx1, yycon1, yycon2,
			yycon3, yycon4, yycon5, dy1ty1, dy2ty1, dy3ty1, dy4ty1, dy5ty1,
			zzcon1, zzcon2, zzcon3, zzcon4, zzcon5, dz1tz1, dz2tz1, dz3tz1,
			dz4tz1, dz5tz1, dnxm1, dnym1, dnzm1, c1c2, c1c5, c3c4, c1345,
			conz1, c1, c2, c3, c4, c5, c4dssp, c5dssp, dtdssp, dttx1, bt,
			dttx2, dtty1, dtty2, dttz1, dttz2, c2dttx1, c2dtty1, c2dttz1,
			comz1, comz4, comz5, comz6, c3c4tx3, c3c4ty3, c3c4tz3, c2iv, con43,
			con16;

	protected double ce[] = { 2.0, 1.0, 2.0, 2.0, 5.0, 0.0, 0.0, 2.0, 2.0, 4.0,
			0.0, 0.0, 0.0, 0.0, 3.0, 4.0, 0.0, 0.0, 0.0, 2.0, 5.0, 1.0, 0.0,
			0.0, 0.1, 3.0, 2.0, 2.0, 2.0, 0.4, 0.5, 3.0, 3.0, 3.0, 0.3, 0.02,
			0.01, 0.04, 0.03, 0.05, 0.01, 0.03, 0.03, 0.05, 0.04, 0.03, 0.02,
			0.05, 0.04, 0.03, 0.5, 0.4, 0.3, 0.2, 0.1, 0.4, 0.3, 0.5, 0.1, 0.3,
			0.3, 0.5, 0.4, 0.3, 0.2 };

	public boolean timeron = false;
	public Timer timer = new Timer();
	public static final int t_total = 1, t_rhsx = 2, t_rhsy = 3, t_rhsz = 4,
			t_rhs = 5, t_xsolve = 6, t_ysolve = 7, t_zsolve = 8, t_rdis1 = 9,
	...
	public void setupThreads(SP sp) {
		...
		for (int ii = 0; ii < num_threads; ii++) {
			rhscomputer[ii] = new RHSCompute(sp, partition1[ii][0],
			...
			txinverse[ii] = new TXInverse(sp, partition2[ii][0],
					partition2[ii][1]);
			txinverse[ii].id = ii;
			//txinverse[ii].start();
			...
			rhsadder[ii] = new RHSAdder(sp, partition2[ii][0],
					partition2[ii][1]);
			rhsadder[ii].id = ii;
			//rhsadder[ii].start();
		}
	...
	public void initialize() {
		...
		double xi, eta, zeta, Pface[] = new double[5 * 3 * 2], Pxi, Peta, Pzeta, temp[] = new double[5];
		...
		for (k = 0; k < grid_points[2]; k++) {
			...
			for (j = 0; j < grid_points[1]; j++) {
				...
				for (i = 0; i < grid_points[0]; i++) {
					...
					for (ix = 0; ix <= 1; ix++) {
						Pxi = ix;
						exact_solution(Pxi, eta, zeta, Pface, 0 + 0 * 5 + ix
								* 15);
					}
					for (iy = 0; iy <= 1; iy++) {
						Peta = iy;
						exact_solution(xi, Peta, zeta, Pface, 0 + 1 * 5 + iy
								* 15);
					}

					for (iz = 0; iz <= 1; iz++) {
						Pzeta = iz;
						exact_solution(xi, eta, Pzeta, Pface, 0 + 2 * 5 + iz
								* 15);
					}

			...
			for (j = lower_bound; j <= upper_bound; j++) {
				for (i = 1; i <= nx2; i++) {
					...
					for (k = 0; k <= nz2 + 1; k++) {
						...
						rhos[k] = dmax1(dz4 + con43 * ru1, dz5 + c1c5 * ru1,
								dzmax + ru1, dz1);
					}
			...
			for (k = lower_bound; k <= upper_bound; k++) {
				for (j = 1; j <= ny2; j++) {
					for (i = 1; i <= nx2; i++) {
						...
						rhs[1 + i * isize1 + j * jsize1 + k * ksize1] = -uzik1
								* r2 + xvel * t2;
						rhs[2 + i * isize1 + j * jsize1 + k * ksize1] = uzik1
								* r1 + yvel * t2;
						rhs[3 + i * isize1 + j * jsize1 + k * ksize1] = zvel
								* t2 + t3;

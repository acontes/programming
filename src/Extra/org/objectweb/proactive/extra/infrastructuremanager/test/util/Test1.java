	...
	public void testPrincipal() {
		try {
			...
			if(!nodes.isEmpty()) {
				...
				assertEquals("freeNodes",monitor.getNumberOfAllResources().intValue()-2,monitor.getNumberOfFreeResource().intValue());

				// Release thoose nodes
				user.freeNodes(nodes);
				Thread.sleep(5000);
				assertEquals("freeNodes After", monitor.getNumberOfAllResources().intValue(), monitor.getNumberOfFreeResource().intValue());
			}
...
//			Get At Most 3 nodes not on fiacre (but there is only 2 nodes corresponding"
			...
			if(!nodes.isEmpty()) {
				...
				assertEquals("getAtMostNodes(3, verif)",nodes.size(),monitor.getNumberOfAllResources().intValue()-monitor.getNumberOfFreeResource().intValue());

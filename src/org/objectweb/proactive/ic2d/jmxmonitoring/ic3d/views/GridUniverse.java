 ...
 * universe and the views. 
 * The sequence for displaying a universe is:
 * <ol>
...
public class GridUniverse {
    ...
    /**
     * Creates a new view branch used for view the scene from 
    ...
    private Quat4f eulerToQuat(double x, double y, double z, double angle){
    	/*
    	 * since quaternions are impossible to visualize
    	 * 	euler to quaternion transform is 
    	 *
    	 * q = [sin(angle/2)*axis, cos(angle/2)]
    	 */
    	return new Quat4f((float) (Math.sin(angle/2)*x),
    ...
    private void setUpAutomaticOrbit(TransformGroup viewPlatformTransform) {
    	...
    	float[]  knots = new float[] {0f,.25f,.50f,.75f,1f}; //for equally distanced knots
    	/*
    	 * since quaternions are impossible to visualize
    	 * 	euler to quaternion transform is 
    	 *
    	 * q = [sin(angle/2)*axis, cos(angle/2)]
    	 * 
    	 * In the following we need the view to be oriented towards
    	...
    	*/
    	...
    	Quat4f[] quats =  new Quat4f[] {
    					  ...
    					  eulerToQuat(0,-DISTANCE,-DISTANCE,Math.PI/100),
    					  eulerToQuat(DISTANCE,0,-DISTANCE,-Math.PI/100),
    					  eulerToQuat(0,DISTANCE,-DISTANCE,Math.PI/100)}; //axes and amount of rotation
    	...
    	RotPosPathInterpolator interp = new RotPosPathInterpolator(alpha,
    						viewPlatformTransform,axisOfTransform,
     ...
     * @param grid the grid figure to be added to the universe
     */
    ...
    /**
     * This method is used by the addGrid method to create 
     * a BranchGroup to which to connect the grid figure. 
    ...
    private BranchGroup createObjectBranch() {
        ...
        objGrp.setCapability(Group.ENABLE_PICK_REPORTING);
        //create background
...
//        transObj.addChild(back);
        ...
        //TODO make class 
                		HoverInfo tooltip = new HoverInfo(universe.getCanvas(),objGrp, new BoundingSphere(new Point3d(0,0,0),10000));
                		objGrp.addChild(tooltip);

        //TODO make class 
        		com.sun.j3d.utils.pickfast.behaviors.PickTranslateBehavior pickRotate = new com.sun.j3d.utils.pickfast.behaviors.PickTranslateBehavior(objGrp,universe.getCanvas(), new BoundingSphere());
        		pickRotate.setSchedulingBounds(new BoundingSphere());
        		objGrp.addChild(pickRotate);
        		com.sun.j3d.utils.pickfast.behaviors.PickTranslateBehavior pickTranslate = new com.sun.j3d.utils.pickfast.behaviors.PickTranslateBehavior(objGrp,universe.getCanvas(), new BoundingSphere());
        		pickTranslate.setSchedulingBounds(new BoundingSphere());
        		objGrp.addChild(pickTranslate);
        ////		PickZoomBehavior pickZoom = new PickZoomBehavior(objGrp,canvas3D, new BoundingSphere());
...
//        objGrp.addChild(coordTrans);
    ...
    /**
     *  Utility method for moving the view platform so
     *  the figures are visible
     * @param trans the view platform's TransformGroup
     */

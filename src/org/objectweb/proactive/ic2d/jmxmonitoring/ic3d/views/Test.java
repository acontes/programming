...
public class Test extends JFrame {
            ...
            new Thread(new Runnable() {
                    public void run() {
                        ...
                        grid.drawArrow(new Double(Math.random()).toString(),
                            ...
                            (long) Math.random() * 5000, obiecte.get(a),
                            obiecte.get(b));
                    }
                }).start();
        }
    ...
    private void testFew() {
        ...
        grid.removeSubFigure("predadab");
        grid.removeSubFigure(pu);
        // grid.addSubFigure("cheypa", ch);
        grid.addSubFigure("predadab", pr);
        grid.removeSubFigure(pr);

        // o4.setState(State.UNKOWN);
        // while (true) {
        // new Thread(new Runnable() {
        // public void run() {
        // int a;
        ...
        // grid.drawArrow(new Double(Math.random()).toString(),
        // new Double(a).toString(),
        // (long) Math.random() * 5000, start, stop);
        // }
        // }).start();
        //
        // // try {

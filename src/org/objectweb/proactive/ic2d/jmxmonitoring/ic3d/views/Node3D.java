...
/**
 * An implementation for a node figure. The figure
 * is a cube 1x1x1  that gets resized when placed on the runtime. 
 * @author vjuresch
 *
 */
public class Node3D extends AbstractNode3D {
    ...
    public void arrangeSubFigures() {
           ...
           for (AbstractFigure3D ao : this.getSubFigures().values()) {
           		PlacementBasket.sphericalAOVerticalArrangement(0.05,1.5, i, this.getSubFigures().size(),
           			ao, this);
           		i++;
           		ao.arrangeSubFigures();
           }
//    		//change the height of the node to fit the number of active objects
//    		TransformGroup scaleHeight = (TransformGroup)this.getParent().getParent();
...
//    		scaleHeight.setTransform(height);
    ...
    protected TransformGroup createTextBranch() {
    	return TextStylesBasket.nodeText(getShortenedName(10));
    }

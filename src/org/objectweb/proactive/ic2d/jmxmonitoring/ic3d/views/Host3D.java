...
/**
 * An implementation of a host figure. The geometry used is a flat 
 * 1x1 square
 * @author vasile
 *
 */
public class Host3D extends AbstractHost3D {
    /**
     * @param name the host name to be displayed
     */
    public Host3D(String name) {
        super(name);
    }
    /*
     * The subfigure is attached to the transform of the
     * figure itself, therefore any placement takes place in 
     * the local coordinate system of the figure. 
     */
    /* (non-Javadoc)
    ...
    public void arrangeSubFigures() {
        ...
        for (AbstractFigure3D runtime : this.getSubFigures().values()) {
        	PlacementBasket.xArrangement(0.1, 0.1, 0.1, i, this.getSubFigures().size(),
        			runtime, this);
        	i++;

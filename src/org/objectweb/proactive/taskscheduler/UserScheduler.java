...
public class UserScheduler extends SchedulerUserAPI{
	...
	public Vector<UserResult> submit(Vector<ProActiveTask> userTasks,String userName) throws UserException
	{
		if(stopped)
...
//		must wrap in a vector because policies only take vectors
		...
		for(int i=0;i<userTasks.size();i++)
		{
			...
			userResultVector.add(new UserResult(tempID,userName,(UserScheduler)ProActive.getStubOnThis()));
	...
	public UserResult submit(ProActiveTask userTask,String userName) throws UserException
	{
		...
		scheduler.submit(task);
		 ...
		 return new UserResult(ID,userName,(UserScheduler)ProActive.getStubOnThis());
		
		
		 
		
	}
	public InternalResult getResult(String taskID1,String userName) throws UserException
	{
		if(stopped)
			throw new UserException("Scheduler is stopped. Please contact the administator");

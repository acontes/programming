		...
		if (keys != null)
		{
			for (int e = 0; e < keys.length; e++)
			{
				...
				if (!int_stack.empty())
				{
					...
					{
						log(
							"Failed to assign an interval to engine "
								+ keys[e]);
		...
		else if (i_left == 0)
		{
			...
			if (previousTime != -1)
				showMessageAll(
					"Speed Up : "
						+ java.lang.Math.rint(
							((double) previousTime / elapsed) * 1000)
							/ 1000
						+ "\n");
			else
				showMessageAll("");
...
//	}
	...
	public void processRotate(
		org.objectweb.proactive.Body body,
		String methodName,
		Request r)
	{
		int i_user = 0;
		...
		if (methodName.equals("rotateLeft"))
		{
			...
			else
			{
				...
				if (i_left > 0)
				{
					showMessage(
						i_user,
						"Rendering in progress, request invalid");
				}
				...
				{
					election =
						new Election(i_user, new Integer(Election.LEFT), this);
				}
			}
		}
		else if (methodName.equals("rotateRight"))
		{
			...
			else
			{
				...
				{
					election =
						new Election(i_user, new Integer(Election.RIGHT), this);
				}
			}
		}
		else if (methodName.equals("rotateUp"))
		{
			...
			else
			{
				...
				{
					election =
						new Election(i_user, new Integer(Election.UP), this);
				}
			}
		}
		else if (methodName.equals("rotateDown"))
		{
			...
			else
			{
				...
				{
					election =
						new Election(i_user, new Integer(Election.DOWN), this);
				}
			}
		}
	...
	{
		((User) h_users.get(new Integer(i_user))).getObject().showMessage(
			s_message);
	}
	...
	{
		((User) h_users.get(new Integer(i_user))).getObject().dialogMessage(
			subject,
			s_message);
	}
	...
	{
		((User) h_users.get(new Integer(i_user))).getObject().showUserMessage(
			s_message);
	}
	...
	public void addSphere(Sphere s)
	{
		...
		else
		{
			...
			showMessageAll(
				"A Sphere has been added\nX:"
					+ Math.floor(s.getCenter().x)
					+ " Y:"
	...
	class C3DDispatcherFrame extends Frame implements ActionListener
	{
		...
		private MenuItem mi_exit, mi_clear, mi_benchmark;
		...
		public C3DDispatcherFrame()
		{
			...
			l_header.setFont(
				new Font("SansSerif", Font.ITALIC + Font.BOLD, 18));
...
class Election extends Thread
{
	...
	public Election(int i_user, Integer wish, C3DDispatcher c3ddispatcher)
	{
		...
		c3ddispatcher.showMessageExcept(
			i_user,
			"New " + WAITSECS + " second election started:");
		c3ddispatcher.showMessageExcept(
			i_user,
			"   User "
				+ c3ddispatcher.nameOfUser(i_user)
				+ " wants to rotate "
				+ voteString(wish));

...
public class CertificateChainValidation {
    ...
    public static void main(String[] args) {
        ...
        try {
            acCert = CertTools.genSelfCert("CN=autorithy", 50, null,
        ...
        try {
            certLevel1 = CertTools.genCert("CN=level1", 50, null,
                    kp1.getPrivate(), kp1.getPublic(), true,
                    acCert.getSubjectDN().toString(), acPrivateKey,
                    acCert.getPublicKey());
        } catch (InvalidKeyException e2) {
            e2.printStackTrace();
        } catch (NoSuchAlgorithmException e2) {
        ...
        System.out.println("Generate level1  cert " +
            certLevel1.getSubjectDN() + " ||  " + certLevel1.getIssuerDN());
        try {
        ...
        try {
            certLevel2 = CertTools.genCert("CN=level2", 50, null,
                    kp2.getPrivate(), kp2.getPublic(), true,
                    certLevel1.getSubjectDN().toString(), privateKeyLevel1,
                    certLevel1.getPublicKey());
        } catch (InvalidKeyException e2) {
            e2.printStackTrace();
        } catch (NoSuchAlgorithmException e2) {
            e2.printStackTrace();
        ...
        System.out.println("Generate level2  cert " +
            certLevel2.getSubjectDN() + " ||  " + certLevel2.getIssuerDN());
        try {
        ...
        try {
            ...
            storin.load(new ByteArrayInputStream(bout.toByteArray()),
                "ha".toCharArray());

...
public class BindingsHandler extends AbstractUnmarshallerDecorator {
    ...
    public BindingsHandler(ComponentsCache componentsCache) {
        ...
        addHandler(ComponentsDescriptorConstants.BINDING_TAG,
            new BindingHandler());
    }
    ...
    public class BindingHandler extends BasicUnmarshaller {
        ...
        private void setClient(String client) {
            ...
            clientInterface = client.substring(client.indexOf('.') + 1,
                    client.length());
        }

        private void setServer(String server) {
            serverComponent = server.substring(0, server.indexOf('.'));
            serverInterface = server.substring(server.indexOf('.') + 1,
                    server.length());
        }
        ...
        public void startContextElement(String name, Attributes attributes)
            ...
            try {
                if (!componentsCache.containsComponentNamed(serverComponent)) {
                    throw new SAXException("tried to perform a binding from " +
                        serverComponent + '.' + serverInterface + " , but " +
                        serverComponent + " does not exist");
                }
                ((BindingController) componentsCache.getComponent(clientComponent)
                                                    .getFcInterface(Constants.BINDING_CONTROLLER)).bindFc(clientInterface,
                    ((Component) componentsCache.getComponent(serverComponent)).getFcInterface(
                        serverInterface));
                logger.debug("**** bound " + clientComponent + "." +
                    clientInterface + " to " + serverComponent + "." +
                    serverInterface);

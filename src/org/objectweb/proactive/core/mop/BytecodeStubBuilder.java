...
public class BytecodeStubBuilder {
    ...
    protected static final Type CLASS_TYPE = convertClassNameToType(
            "java.lang.Class");
    protected static final Type CLASS_ARRAY_TYPE = new ArrayType(CLASS_TYPE, 1);
    protected static final Type OBJECT_TYPE = convertClassNameToType(
            "java.lang.Object");
    protected static final Type OBJECT_ARRAY_TYPE = new ArrayType(OBJECT_TYPE, 1);
    protected static final Type METHOD_TYPE = convertClassNameToType(
            "java.lang.reflect.Method");
    protected static final Type METHOD_ARRAY_TYPE = new ArrayType(METHOD_TYPE, 1);
    protected static final Type PROXY_TYPE = convertClassNameToType(
            "org.objectweb.proactive.core.mop.Proxy");
    protected static final Type METHODCALL_TYPE = convertClassNameToType(
    ...
    protected MethodGen createMethod(int methodIndex, Method m) {
        ...
        if (this.cl.isInterface() == false) {
            ...
            instructionList.append(factory.createGetField(
                    this.stubClassFullName, "outsideConstructor", Type.BOOLEAN));
        }
        ...
        instructionList.append(factory.createGetField(this.stubClassFullName,
                PROXY_FIELD_NAME, PROXY_TYPE));

        // Pushes on the stack the Method object that represents the current method
        instructionList.append(factory.createGetStatic(this.stubClassFullName,
                "methods", METHOD_ARRAY_TYPE));
        instructionList.append(new PUSH(this.classGenerator.getConstantPool(),
                methodIndex));
        ...
        instructionList.append((Instruction) factory.createNewArray(
                OBJECT_TYPE, (byte) 1));
        ...
        for (int i = 0; i < paramTypes.length; i++) {
            ...
            if (paramTypes[i].isPrimitive()) {
                ...
                instructionList.append(InstructionFactory.createLoad(theType,
                        indexInParameterArray));
                // Handles the fact that some primitive types need 2 slots
                indexInParameterArray = indexInParameterArray +
                    lengthOfType(paramTypes[i]);

                // Now, we call the constructor of the wrapper
                Type[] argtypes = new Type[] { convertClassToType(paramTypes[i]) };
                instructionList.append(factory.createInvoke(nameOfWrapper,
                        "<init>", Type.VOID, argtypes, Constants.INVOKESPECIAL));
            } else {
            ...
            instructionList.append(InstructionFactory.createArrayStore(
                    OBJECT_TYPE));
        }
        ...
        instructionList.append(factory.createInvoke(
                "org.objectweb.proactive.core.mop.MethodCall", "getMethodCall",
                METHODCALL_TYPE, new Type[] { METHOD_TYPE, OBJECT_ARRAY_TYPE },
                Constants.INVOKESTATIC));

        // Now, call 'reify' on the proxy object
        instructionList.append(factory.createInvoke(
                "org.objectweb.proactive.core.mop.Proxy", "reify", Type.OBJECT,
                new Type[] { METHODCALL_TYPE }, Constants.INVOKEINTERFACE));
        ...
        if (m.getReturnType().isPrimitive()) {
            ...
            instructionList.append(factory.createCheckCast(
                    (ReferenceType) convertClassToType(m.getReturnType())));
        }
        ...
        if (this.cl.isInterface() == false) {
            ...
            instructionList.insert(outsideConstructorHandle,
                InstructionFactory.createBranchInstruction(Constants.IFEQ,
                    inConstructorHandle));
            ...
            for (int i = 0; i < paramTypes.length; i++) {
                Type theType = convertClassToType(paramTypes[i]);
                LocalVariableInstruction lvi = InstructionFactory.createLoad(theType,
                        indexInParameterArray);
            ...
            String declaringClassName = this.methods[methodIndex].getDeclaringClass()
                                                                 .getName();
            instructionList.append(factory.createInvoke(declaringClassName,
                    m.getName(), convertClassToType(m.getReturnType()),
                    convertClassArrayToTypeArray(paramTypes),
                    Constants.INVOKESPECIAL));

            // Returns the result to the caller
            InstructionHandle returnHandle = instructionList.append(InstructionFactory.createReturn(
            ...
            instructionList.append(InstructionFactory.createReturn(
                    convertClassToType(m.getReturnType())));
        }
    ...
    protected void createUnwrappingCode(InstructionFactory factory, Class c) {
        ...
        if (c.equals(Void.TYPE)) {
            ...
            instructionList.append(factory.createCheckCast(
                    (ReferenceType) convertClassNameToType(nameOfWrapperClass)));
            // And then perform the call
            instructionList.append(factory.createInvoke(nameOfWrapperClass,
                    nameOfPrimitiveType + "Value", convertClassToType(c),
    ...
    protected void createConstructor() {
        ...
        if (!this.cl.isInterface()) {
            ...
            instructionList.append(factory.createInvoke(this.className,
            ...
            // Calls the constructor of the super class
            instructionList.append(InstructionFactory.createLoad(Type.OBJECT, 0));
            instructionList.append(factory.createInvoke("java.lang.Object",
                    "<init>", Type.VOID, Type.NO_ARGS, Constants.INVOKESPECIAL));
        }
    ...
    protected void createStaticInitializer() {
        ...
        MethodGen mg = new MethodGen(Constants.ACC_STATIC, // Static method
                ...
                new String[0], Constants.STATIC_INITIALIZER_NAME,
                this.stubClassFullName, instructionList,
                this.classGenerator.getConstantPool());
        ...
        instructionList.append(factory.createPutStatic(this.stubClassFullName,
                "methods", METHOD_ARRAY_TYPE));
        ...
        for (int i = 0; i < vectorOfSuperClasses.size(); i++) {
            ...
            instructionList.append(new PUSH(
                    this.classGenerator.getConstantPool(), i));
            // Loads the name of the class onto the stack
            String s = ((Class) vectorOfSuperClasses.elementAt(i)).getName();
            instructionList.append(new PUSH(
                    this.classGenerator.getConstantPool(), s));
            ...
            instructionList.append(factory.createInvoke("java.lang.Class",
                    "forName", CLASS_TYPE, argstypes, Constants.INVOKESTATIC));
        ...
        for (int i = 0; i < this.methods.length; i++) {
            ...
            instructionList.append(factory.createGetStatic(
                    this.stubClassFullName, "methods", METHOD_ARRAY_TYPE));
            instructionList.append(new PUSH(
                    this.classGenerator.getConstantPool(), i));
            ...
            instructionList.append(new PUSH(
                    this.classGenerator.getConstantPool(), indexInClassArray));
            instructionList.append(InstructionFactory.createArrayLoad(
                    CLASS_TYPE));
            ...
            instructionList.append(new PUSH(
                    this.classGenerator.getConstantPool(),
                    this.methods[i].getParameterTypes().length));
            // Creates an array of class objects of that size
            instructionList.append((Instruction) factory.createNewArray(
                    CLASS_TYPE, (byte) 1));
            ...
            for (int j = 0; j < this.methods[i].getParameterTypes().length;
                ...
                instructionList.append(InstructionFactory.createLoad(
                        Type.OBJECT, 2));
                // Load the index in the array where we want to store the result
                instructionList.append(new PUSH(
                        this.classGenerator.getConstantPool(), j));
                ...
                if (currentParameter.isPrimitive()) {
                    ...
                    instructionList.append(factory.createGetStatic(
                            Utils.getWrapperClass(currentParameter).getName(),
                            "TYPE", CLASS_TYPE));
                } else {
                    ...
                    argstypes[0] = Type.STRING;
                    instructionList.append(factory.createInvoke(
                            "java.lang.Class", "forName", CLASS_TYPE,
                            argstypes, Constants.INVOKESTATIC));
                }
            ...
            instructionList.append(factory.createInvoke("java.lang.Class",
                    "getDeclaredMethod", METHOD_TYPE, argstypes,
    ...
    protected void createGetAndSetProxyMethods() {
        ...
        instructionList.append(InstructionFactory.createLoad(Type.OBJECT, 0));
        instructionList.append(InstructionFactory.createLoad(Type.OBJECT, 1));
        instructionList.append(factory.createPutField(this.stubClassFullName,
                PROXY_FIELD_NAME, PROXY_TYPE));
    ...
    protected static Type convertClassToType(Class cl) {
        if (cl.isPrimitive()) {
            ...
            if (cl.isArray()) {
                return new ArrayType(convertClassToType(cl.getComponentType()),
                    1);
            } else {

...
public class ASMBytecodeStubBuilder implements Constants {
    ...
    public ASMBytecodeStubBuilder(String classname)
    ...
    protected CodeVisitor createMethod(int methodIndex, Method m) {
        ...
        if (this.cl.isInterface() == false) {
            ...
            cv.visitFieldInsn(GETFIELD,
                this.stubClassFullName.replace('.', '/'), "outsideConstructor",
                "Z");
        ...
        if (this.cl.isInterface() == false) {
            ...
            for (int i = 0; i < paramTypes.length; ++i) {
                cv.visitVarInsn(ILOAD + getOpcodeOffset(paramTypes[i]),
            ...
            cv.visitMethodInsn(INVOKESPECIAL,
                declaringClassName.replace('.', '/'), m.getName(),
    ...
    protected void createFields() {
        ...
        if (!this.cl.isInterface()) {
            this.classGenerator.visitField(ACC_PROTECTED, "outsideConstructor",
                "Z", null, null);
        }
    ...
    protected void createConstructor() {
        ...
        if (!this.cl.isInterface()) {
            ...
            cv.visitVarInsn(ALOAD, 0);
            cv.visitMethodInsn(INVOKESPECIAL, this.className.replace('.', '/'),
                "<init>", "()V");

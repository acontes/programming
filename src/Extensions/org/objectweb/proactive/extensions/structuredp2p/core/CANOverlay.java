 ...
 * provides hash table-like functionality on Internet-like scales. The CAN is
...
public class CANOverlay extends StructuredOverlay {
	...
	/**
	 * Neighbors of the current area. The neighbors are an array of ProActive
	 * groups.
	 */
	...
	public CANOverlay(Peer localPeer) {
		...
		try {
			...
			for (i = 0; i < NB_DIMENSIONS; i++) {
				this.neighbors[i][0] = PAGroup.getGroup((Peer) PAGroup
						.newGroup(Peer.class.getName()));
				this.neighbors[i][1] = PAGroup.getGroup((Peer) PAGroup
						.newGroup(Peer.class.getName()));
			}
	...
	/**
	 * Splits the current area in two. The axe which is used in order to split
	 * is choose randomly with {@link #getRandomAxe()}.
	 * 
	 * @param peer
	...
	public void split(Peer peer) {
		...
		if (!response.isNull()) {
			...
			/*
			 * Coordinate[] middle =
			 * this.getMiddleArea(this.getRandomDimension()); Area newArea = new
	 ...
	 * @return 0 if the coordinate is contained by the area on the given axe, -1
	 *         if the coordinate is smaller than the line which is managed by
	 *         the given axe, 1 otherwise.
	 */
	public int contains(int axeIndex, Coordinate coordinate) {
		...
		if (minArea[axeIndex].getValue().compareTo(coordinate.getValue()) > 0
				&& maxArea[axeIndex].getValue()
						.compareTo(coordinate.getValue()) <= 0) {
			return 0;
		} else if (minArea[axeIndex].getValue()
				.compareTo(coordinate.getValue()) > 0) {
			return -1;
		} else {
			return 1;
	...
	/**
	 * Merge two area when a peer leave the network. The split consists to give
	 * the data that are managed by the peer which left the network to his
	 * neighbors and after to merge this area with its closest neighbors.
	 * 
	 * @param peer
	...
	public Boolean join(Peer remotePeer) {
		...
		if (this.addNeighbor(remotePeer, dim, 1).booleanValue()
				&& ((JoinResponseMessage) this.getLocalPeer().sendMessageTo(
						remotePeer,
						new CANJoinMessage(this.getRemotePeer(), dim, 0)))
						.hasSucceeded()) {
	...
	public CANLookupResponseMessage handleLookupMessage(LookupMessage msg) {
		return new CANLookupResponseMessage(this.getLocalPeer(),
	...
	public CANJoinResponseMessage handleJoinMessage(Message msg) {
		...
		return new CANJoinResponseMessage(this.addNeighbor(message.getPeer(),
				message.getDimesion(), message.getOrder()));

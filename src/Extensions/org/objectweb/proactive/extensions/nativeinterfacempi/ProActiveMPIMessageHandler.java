		...
		try {
			...
			if (m_r.getMsgType() == ProActiveNativeConstants.COMM_MSG_INIT) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug(prefix
							+ " [START] sending message to remote mpi\n"
							+ m_r.toString(prefix));
				}
				...
				if (logger.isDebugEnabled()) {
					logger.debug(prefix
							+ " [START] Scatter message processing\n"
							+ m_r.toString(prefix));
				}
				...
				if (logger.isDebugEnabled()) {
					logger.debug(prefix
							+ " [END]  Scatter message processing\n"
							+ m_r.toString(prefix));
				}
				...
				if (logger.isDebugEnabled()) {
					logger.debug(prefix
							+ " [START] Bcast message processing\n"
							+ m_r.toString(prefix));
				}
				...
				if (logger.isInfoEnabled()) {
					logger.info(prefix + " msg_type UNKNOWN  "
							+ m_r.toString(prefix));
				}
			}
		} catch (Exception e) {
			...
			if (logger.isDebugEnabled()) {
				logger
						.debug(prefix + " exception in Message recv handler "
								+ e);
			}
			e.printStackTrace();
		}

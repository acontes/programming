    ...
    public void deploy(List<NativeSpmd> spmdList) {
        ...
        try {
            for (int i = 0; i < spmdList.size(); i++) {
                ...
                java.net.URL u = cl
                        .getResource(this.getClass().getPackage().toString().replace('.', '/') + FULL_LIBRARY_NAME);
                ...
                for (int j = 0; j < params.length; j++) {
                    params[j] = new Object[] { LIBRARY_NAME,
                            (ProActiveNativeManager) PAActiveObject.getStubOnThis(), currentNumberOfJob, j, spmd.getFactory() };
                ...
                InboundProxy inboundProxyGroup = (InboundProxy) PASPMD.newSPMDGroup(
                        InboundProxy.class.getName(), params, allNodes);
                ...
                OutboundProxy outboundProxyGroup = (OutboundProxy) PASPMD.newSPMDGroup(OutboundProxy.class.getName(), params2, allNodes);
    ...
    public void register(int jobID, int rank) {
        ...
        if (logger.isInfoEnabled()) {
            logger.info("[MANAGER] JobID #" + jobID + " rank " + rank +
                ...
                (this.inboundProxyArrayMap.get(jobID).length - ackToRecv[jobID]) + "/" + this.inboundProxyArrayMap.get(jobID).length +
                ")");
        }
            ...
            for (int i = 0; i < currentNumberOfJob; i++) {
                ...
                try {
                	...
                	for (int j = 0; j < tab.length; j++) {
                		...
                		tab[j].notifyProxy(this.inboundProxyArrayMap,
                				this.inboundProxyMap, outboundProxyStub);
                	}
                } catch (Exception e) {
                    e.printStackTrace();
                }

...
public class Task<T> implements Serializable, Comparable<Task> {
    ...
    private Task(T object, int id, int priority, int intraFamilyPriority,
        int parentId, int familyId) {
        this.param = object;
        this.id = id;
    ...
    public Task(T object) {
        this(object, (int) (Math.random() * Integer.MAX_VALUE),
            DEFAULT_PRIORITY, DEFAULT_INTRA_FAMILY_PRIORITY,
    ...
    public <R> Task<R> reBirth(R object) {
        Task<R> newMe = new Task<R>(object, id, priority, intraFamilyPri,
                parentId, familyId);
    ...
    public synchronized boolean setFinishedChild(Task<?> task) {
        ...
        if (task.getParentId() != this.getId()) {
            logger.error("Setting other task's child as my child: child.id=" +
                " task.parent.id=" + task.parentId);
            return false; //not my child
        }

        if (!this.childrenWaiting.containsKey(task)) {
            logger.error("Parent id=" + this.id +
                " not waiting for child: task.id=" + task.id);
            return false;
        }
    ...
    public synchronized boolean isReady() {
        return childrenReady.isEmpty() && childrenWaiting.isEmpty();
    }
    ...
    public synchronized Vector<Task<T>> getFinishedChildren() {
        Vector<Task<T>> res = childrenFinished;
        childrenFinished = new Vector<Task<T>>();
        intraFamilyPri = DEFAULT_INTRA_FAMILY_PRIORITY;

...
public class SimpleObjectMigration implements Serializable {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("SimpleObjectMigration> main() > " +
            "We are going to try to migrate a simple object from " + urlSourceNode + " to " +
            urlDestinationNode);

        try {
            logger.info("SimpleObjectMigration> main() > " + "we try to get the source node : " +
                urlSourceNode);
            ...
            logger.info("SimpleObjectMigration> main() > " + "Exception during the getting of " +
                " the source node " + urlSourceNode + " (" + e.getMessage() + ")");
            e.printStackTrace();
        }

        try {
            logger.info("SimpleObjectMigration> main() > " + "we try to get the destination node : " +
                urlDestinationNode);

            destinationNode = NodeFactory.getNode(urlDestinationNode);

            logger.info("SimpleObjectMigration> main() > " + "we obtain the destination node : " +
                urlDestinationNode);
        } catch (NodeException e) {
            System.out.println("SimpleObjectMigration> main() > " + "Exception during the getting of " +
                " the destination node " + urlDestinationNode + " (" + e.getMessage() + ")");
            e.printStackTrace();
        }

        logger.info("SimpleObjectMigration> main() > " + "We shows the state before"
        ...
        try {
            ...
            activeHello = (SimpleObjectMigration) PAActiveObject.newActive(className, params, sourceNode);
            ...
            logger.info("SimpleObjectMigration> main() > " +
                "Exception during the creation of the active object" + " (" + e.getMessage() + ")");
            e.printStackTrace();
        } catch (NodeException e) {
            logger.info("SimpleObjectMigration> main() > " +
                "Exception during the creation of the active object" + " (" + e.getMessage() + ")");
        ...
        logger.info("SimpleObjectMigration> main() > " + "We show the state before"
        ...
        logger.info("SimpleObjectMigration> main() > " + "We show the state after"
            + " the migration of the Active Object");

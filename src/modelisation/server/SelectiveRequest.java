...
public class SelectiveRequest extends RequestImpl
    ...
    // -- CONSTRUCTORS -----------------------------------------------
    //
    public SelectiveRequest(MethodCall methodCall, UniversalBody sender,
        boolean isOneWay, long nextSequenceID, MicroTimer microTimer) {
    ...
    public Reply serve(Body targetBody) throws ServeException {
        ...
        if (!isOneWay && (sender != null)) {
            ...
            if ("searchObject".equals(methodName)) {
                if (sendAnswer(targetBody, "updateLocation")) {
                    ...
                    try {
                        System.out.println(
                            "SelectiveService: puting the request back in the line");
        ...
        System.out.println("SelectiveService: .............. done after " +
            endTime + " for method " + methodName);
        return reply;
    }

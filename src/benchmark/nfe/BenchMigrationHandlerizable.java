...
public class BenchMigrationHandlerizable extends ProActiveBenchmark {
    private A a = null;
    private int nb_handlers;
    private int nb_handlers_tmp = 0;
     ...
     * @param node
     */
    public BenchMigrationHandlerizable(Node node_creation, Node node_dest,
        int nb_handlers, String class_file) {
    ...
    public long action() throws Exception {
        System.out.print("Object protected before migration with " +
            a.getHandlerNumber() + " " + file);
        ...
        /*
           ...
           s.close();
         */
        System.out.println(" / after migration with " + a.getHandlerNumber() +
            " " + file);

        // Generate Benchmarks (GNUplot) for iteration tests  
        FileWriter fw = new FileWriter("MIGRATION to " +
                node_dest.getNodeInformation().getURL().replace('/', ' ') +
                " WITH " + file + ".test", true);
        ...
        while ((nb_handlers_tmp--) >= 1) {
            try {
                ProActive.setExceptionHandler(HandlerNonFunctionalException.class,
                    Class.forName("benchmark.nfe." + file + nb_handlers_tmp)
                         .newInstance().getClass(), Handler.ID_Body, a);
            } catch (ClassNotFoundException e) {
                System.out.println("NFE Class generNFE" + nb_handlers_tmp +
                    " not found : " + e.toString());
            } catch (IllegalAccessException e) {
                System.out.println("NFE Class generNFE" + nb_handlers_tmp +
                    " cannot be accessed : " + e.toString());
            } catch (InstantiationException e) {
                System.out.println("NFE Class generNFE" + nb_handlers_tmp +
                    " cannot be instantiated : " + e.toString());
            }
        }
    }

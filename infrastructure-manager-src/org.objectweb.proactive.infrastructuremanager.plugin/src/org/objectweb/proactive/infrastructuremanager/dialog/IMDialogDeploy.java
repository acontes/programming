				...
				if (checkedItemsNumber == 0) {
					...
					try {
						if (checkedItemsNumber == itemsNumber) {
							admin.deployAllVirtualNodes(new File(filePath.getText()),
									IMConstants.nodeTransfert);
						} else if (checkedItemsNumber == 1) {
							admin.deployVirtualNode(new File(filePath.getText()), IMConstants.nodeTransfert,
									listOfCheckedItem.get(0));
						} else {
							String[] tab = new String[checkedItemsNumber];
							listOfCheckedItem.toArray(tab);
							admin.deployVirtualNodes(new File(filePath.getText()), IMConstants.nodeTransfert,
									tab);
						}
					} catch (Exception e1) {
						e1.printStackTrace();
